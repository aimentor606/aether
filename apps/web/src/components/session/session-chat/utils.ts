...
export function buildForkPrompt(parts: Part[], text?: string): PromptPart[] {
	...
	const value =
		text ??
		parts
			.find((part): part is TextPart => isTextPart(part) && !part.synthetic && !part.ignored)
			?.text ??
		"";
...
export const PTY_EXITED_BLOCK_REGEX =
	/<pty_exited>[\s\S]*?<\/pty_exited>/gi;
	...
	const cleanText = text
		.replace(DCP_TAG_REGEX, (_, attrs: string, body: string) => {
			...
			const tokensSaved = parseInt(
				parseAttr(attrs, "tokens-saved") || "0",
				10,
			);
			const batchSaved = parseInt(
				parseAttr(attrs, "batch-saved") || "0",
				10,
			);
			const prunedCount = parseInt(
				parseAttr(attrs, "pruned-count") || "0",
				10,
			);
	...
	for (const cmd of commands) {
		...
		if (tpl.length > 2000) {
			...
			if (tplBody.length > 0 && trimmedRawText.startsWith(tplBody)) {
				...
				return {
					name: cmd.name,
					args:
						after.length > 0 && after.length < 200 ? after : undefined,
				};
			}
			continue;
		}
		...
		if (trimmedRawText.startsWith(prefix)) {
			...
			if (placeholderMatch) {
				...
				const lastNewlineBlock = afterPrefix
					.split("\n\n")
					.pop()
					?.trim();
