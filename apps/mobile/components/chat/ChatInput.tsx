...
export const ChatInput = React.memo(
  React.forwardRef<ChatInputRef, ChatInputProps>(
    (
      ...
      React.useEffect(() => {
        if (isAgentRunning) {
          pulseOpacity.value = withRepeat(withTiming(0.85, { duration: 1500 }), -1, true);
        } else {
      ...
      const handleButtonPress = React.useCallback(() => {
        ...
        log.log('[ChatInput] 🔘 Button pressed!', {
          isAgentRunning,
          isRecording,
          hasContent,
          hasAgent,
        ...
        if (isAgentRunning || isSendingMessage || isTranscribing) {
          log.log(
            '[ChatInput] 🛑 Calling onStopAgentRun (isAgentRunning:',
            isAgentRunning,
            ', isSendingMessage:',
            isSendingMessage,
            ')'
...
const RecordingMode = React.memo(
  ({
    ...
    <>
      <View className="bottom-5 flex-1 items-center justify-center">
        <AudioWaveform isRecording={true} audioLevels={audioLevels} />
      </View>
      <View className="absolute bottom-6 right-16 items-center">
        <Text className="font-roobert-medium text-xs text-foreground/50">
          {recordingStatusText}
        </Text>
      </View>
      ...
      <View className="mt-3 flex-row items-center justify-between">
        ...
        <View className="flex-row items-center gap-1">
          ...
          <TouchableOpacity
            ...
            disabled={
              isStopping || hasUploadingFiles || (!hasAgent && !isAgentRunning && !isSendingMessage)
            }
            style={{
              width: 40,
              height: 40,
              ...
              justifyContent: 'center',
              opacity:
                isStopping || hasUploadingFiles
                  ? 0.5
                  : !hasAgent && !isAgentRunning && !isSendingMessage
                    ? 0.4
                    : 1,
            }}
            className={
              isAgentRunning || isSendingMessage || isTranscribing || isStopping
                ? 'bg-foreground'
                : 'bg-primary'
            }
            hitSlop={ANDROID_HIT_SLOP}
