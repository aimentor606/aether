...
export default function AdminStressTestPage() {
  ...
  const visibleResults = useMemo(() => {
    ...
    const completed = state.results
      ...
      .sort(
        (a, b) =>
          (b.total_ttft || b.request_time || 0) -
          (a.total_ttft || a.request_time || 0),
      );

    const combined = [
      ...running,
      ...completed.slice(0, Math.max(0, 20 - running.length)),
    ];
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-md text-muted-foreground mt-2">
              Run load tests with detailed timing breakdown (bypasses concurrent
              run limits)
            </p>
          </div>
        </div>
                ...
                <div className="min-w-[600px]">
                  ...
                  <div className="mb-3">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xs font-medium text-purple-600 dark:text-purple-400 w-32">
                        Total TTFT
                      </span>
                  ...
                  <div className="flex gap-1 mb-4">
                    ...
                    <div className="flex-1 flex gap-0.5">
                      <div className="flex-1 h-6 bg-blue-200 dark:bg-blue-900/40 rounded-sm flex items-center justify-center border border-blue-300 dark:border-blue-800/50">
                        <span className="text-[10px] text-blue-700 dark:text-blue-300 truncate px-1">
                          HTTP Call
                        </span>
                      </div>
                      <div className="flex-1 h-6 bg-blue-200 dark:bg-blue-900/40 rounded-sm flex items-center justify-center border border-blue-300 dark:border-blue-800/50">
                    ...
                    <div className="flex-1 flex gap-0.5">
                      <div className="flex-[2] h-6 bg-orange-200 dark:bg-orange-900/40 rounded-sm flex items-center justify-center border border-orange-300 dark:border-orange-800/50">
                        <span className="text-[10px] text-orange-700 dark:text-orange-300 truncate px-1">
                          Agent Setup
                        </span>
                      </div>
                      <div className="flex-[3] h-6 bg-green-200 dark:bg-green-500/20 rounded-sm flex items-center justify-center border border-green-400 dark:border-green-500/40">
                        <span className="text-[10px] text-green-700 dark:text-green-300 font-medium truncate px-1">
                          LLM TTFT
                        </span>
                      </div>
                    </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium">
                      {stats.completed}/{stats.total} requests
                    </span>
                  </div>
              ...
              <CardContent>
                      ...
                      {visibleResults.map((result) => {
                        ...
                        return (
                          <tr key={result.request_id} className="border-b">
                            <td className="h-12 px-4 text-sm font-mono">
                              {result.request_id}
                            </td>
                            ...
                            <td className="h-12 px-4 text-sm text-right font-mono">
                              {result.request_time > 0
                                ? `${result.request_time.toFixed(2)}s`
                                : '-'}
                            </td>
                            <td className="h-12 px-4 text-sm text-right font-mono">
                              {result.time_to_first_response != null ? (
                                <span className="text-orange-500">
                                  {result.time_to_first_response.toFixed(2)}s
                                </span>
                ...
                {stats.total > 20 && (
                  <p className="text-sm text-muted-foreground mt-2 text-center">
                    Showing {Math.min(20, visibleResults.length)} of{' '}
            ...
            <CardContent>
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-2xl font-bold text-red-500">
                    ...
                    <span className="text-sm font-normal text-muted-foreground ml-1">
                      (
                      {(
                        (state.summary.failed / state.summary.total_requests) *
                        100
                      ).toFixed(1)}
                      %)
                    </span>
              ...
              <div className="border-t mt-6 pt-6">
                ...
                <p className="text-xs text-muted-foreground mb-4">
                  Time for HTTP request to complete (distributed across workers
                  like real traffic)
                </p>
                ...
                <div className="border-t mt-6 pt-6">
                  <h4 className="text-sm font-medium mb-2 flex items-center gap-2">
                    ...
                    <Badge variant="secondary" className="ml-2">
                      {state.summary.first_response_measured} measured
                    </Badge>
                  </h4>
                  <p className="text-xs text-muted-foreground mb-4">
                    Time from agent start until first LLM response chunk
                    (includes MCP init, prompt building, LLM TTFT)
                  </p>
                  <div className="grid grid-cols-3 gap-6">
                    <div className="space-y-1">
                      <p className="text-sm text-muted-foreground">Min</p>
                      <p className="text-xl font-semibold text-orange-500">
                        {state.summary.min_time_to_first_response != null
                          ? `${state.summary.min_time_to_first_response}s`
                          : '-'}
                      </p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-sm text-muted-foreground">Average</p>
                      <p className="text-xl font-semibold text-orange-500">
                        {state.summary.avg_time_to_first_response != null
                          ? `${state.summary.avg_time_to_first_response}s`
                          : '-'}
                      </p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-sm text-muted-foreground">Max</p>
                      <p className="text-xl font-semibold text-orange-500">
                        {state.summary.max_time_to_first_response != null
                          ? `${state.summary.max_time_to_first_response}s`
                          : '-'}
                      </p>
                    </div>
                  </div>
              ...
              {state.summary.llm_ttft_measured > 0 && (
                <div className="border-t mt-6 pt-6">
                  <h4 className="text-sm font-medium mb-2 flex items-center gap-2">
                    ...
                    >
                      {state.summary.llm_ttft_measured} measured
                    </Badge>
                  </h4>
                  <p className="text-xs text-muted-foreground mb-4">
                    Actual time for the LLM API call to return first token (from
                    litellm.acompletion call to first chunk)
                  </p>
                  <div className="grid grid-cols-3 gap-6">
                    <div className="space-y-1">
                      <p className="text-sm text-muted-foreground">Min</p>
                      <p className="text-xl font-semibold text-green-500">
                        {state.summary.min_llm_ttft != null
                          ? `${state.summary.min_llm_ttft}s`
                          : '-'}
                      </p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-sm text-muted-foreground">Average</p>
                      <p className="text-xl font-semibold text-green-500">
                        {state.summary.avg_llm_ttft != null
                          ? `${state.summary.avg_llm_ttft}s`
                          : '-'}
                      </p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-sm text-muted-foreground">Max</p>
                      <p className="text-xl font-semibold text-green-500">
                        {state.summary.max_llm_ttft != null
                          ? `${state.summary.max_llm_ttft}s`
                          : '-'}
                      </p>
                    </div>
                  </div>
              ...
              {state.summary.min_total_ttft != null && (
                <div className="border-t mt-6 pt-6">
                  ...
                  <p className="text-xs text-muted-foreground mb-4">
                    Complete time from user request until first response =
                    Request Time + Time to First Response
                  </p>
                  <div className="grid grid-cols-3 gap-6">
                    <div className="space-y-1">
                      <p className="text-sm text-muted-foreground">Min</p>
              ...
              {Object.keys(state.summary.error_breakdown).length > 0 && (
                <div className="border-t mt-6 pt-6">
                  ...
                  <div className="space-y-2">
                    {Object.entries(state.summary.error_breakdown).map(
                      ([error, count]) => (
                        <div
                          key={error}
              ...
              {state.summary.timing_breakdown &&
                Object.keys(state.summary.timing_breakdown).length > 0 && (
                  <div className="border-t mt-6 pt-6">
                    <h4 className="text-sm font-medium mb-2 flex items-center gap-2">
                    ...
                    <p className="text-xs text-muted-foreground mb-4">
                      Detailed breakdown of time spent in each phase during
                          ...
                          {state.summary.timing_breakdown.create_project_ms && (
                            <tr className="border-b border-border/50">
                              ...
                              <td className="text-right py-2 px-2 font-mono">
                                {
                                  state.summary.timing_breakdown
                                    .create_project_ms.min
                                }
                              </td>
                              <td className="text-right py-2 px-2 font-mono">
                                {
                                  state.summary.timing_breakdown
                                    .create_project_ms.avg
                                }
                              </td>
                              <td className="text-right py-2 px-2 font-mono">
                                {
                                  state.summary.timing_breakdown
                                    .create_project_ms.max
                                }
                              </td>
                            </tr>
                          )}
                          {state.summary.timing_breakdown.create_thread_ms && (
                            <tr className="border-b border-border/50">
                              ...
                              <td className="text-right py-2 px-2 font-mono">
                                {
                                  state.summary.timing_breakdown
                                    .create_thread_ms.min
                                }
                              </td>
                              <td className="text-right py-2 px-2 font-mono">
                                {
                                  state.summary.timing_breakdown
                                    .create_thread_ms.avg
                                }
                              </td>
                              <td className="text-right py-2 px-2 font-mono">
                                {
                                  state.summary.timing_breakdown
                                    .create_thread_ms.max
                                }
                              </td>
                            </tr>
                          )}
                          {state.summary.timing_breakdown
                            .create_message_and_run_ms && (
                            <tr className="border-b border-border/50">
                              ...
                              <td className="text-right py-2 px-2 font-mono">
                                {
                                  state.summary.timing_breakdown
                                    .create_message_and_run_ms.min
                                }
                              </td>
                              <td className="text-right py-2 px-2 font-mono">
                                {
                                  state.summary.timing_breakdown
                                    .create_message_and_run_ms.avg
                                }
                              </td>
                              <td className="text-right py-2 px-2 font-mono">
                                {
                                  state.summary.timing_breakdown
                                    .create_message_and_run_ms.max
                                }
                              </td>
                            </tr>
                          )}
