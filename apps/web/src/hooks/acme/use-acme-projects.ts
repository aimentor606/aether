 ...
 * active sandbox route (/v1/p/.../8000/acme/projects). This keeps Acme
...
export interface AcmeProjectDetail extends AcmeProject {
  delegations: Array<{
    session_id: string;
    project_id: string;
    prompt: string;
...
async function acmeFetch<T>(serverUrl: string, apiPath: string, init?: RequestInit): Promise<T> {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Acme API ${res.status}: ${text.slice(0, 100)}`);
  }
  return res.json();
}
...
export function useAcmeProjects() {
  const { user, isLoading: isAuthLoading } = useAuth();
  const serverVersion = useServerStore((s) => s.serverVersion);
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  return useQuery<AcmeProject[]>({
    queryKey: [...acmeKeys.projects(), user?.id ?? 'anonymous', serverUrl, serverVersion],
    queryFn: () => acmeFetch<AcmeProject[]>(serverUrl, ''),
...
export function useAcmeProject(id: string) {
  const { user, isLoading: isAuthLoading } = useAuth();
  const serverVersion = useServerStore((s) => s.serverVersion);
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  return useQuery<AcmeProjectDetail>({
    queryKey: [...acmeKeys.project(id), user?.id ?? 'anonymous', serverUrl, serverVersion],
    queryFn: () => acmeFetch<AcmeProjectDetail>(serverUrl, `/${encodeURIComponent(id)}`),
 ...
 */
export function useAcmeProjectSessions(projectId: string) {
  const { user, isLoading: isAuthLoading } = useAuth();
  const serverVersion = useServerStore((s) => s.serverVersion);
  const serverUrl = useServerStore((s) => s.getActiveServerUrl());
  return useQuery<any[]>({
...
export function useUpdateProject() {
  ...
  return useMutation({
    mutationFn: ({ id, ...data }: { id: string; name?: string; description?: string }) =>
      acmeFetch<AcmeProject>(serverUrl, `/${encodeURIComponent(id)}`, {
        method: 'PATCH',
