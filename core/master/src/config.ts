...
export const config = {
  //master Master port (main entry point)
  ...
  // ───master Backend ─────────────────────────────────────────────────────────
  // AETHER_API_URL: base URL ofapi. Source of truth is the secrets-manager-
  ...
  // AETHER_TOKEN — direction: sandbox →api.
  ...
  // This is howapi (and other external callers) authenticates TO the sandbox.
  ...
  // Counterpart: AETHER_TOKEN goes the other direction (sandbox →api).
  // Auto-generates if not provided — external access is ALWAYS auth-protected.
  // In normal operation,api injects the key as a Docker env var.
  get INTERNAL_SERVICE_KEY(): string {
    const s6EnvDir = process.env.S6_ENV_DIR || '/run/s6/container_environment'
    // Always re-read from s6 env dir first —api may have written it
