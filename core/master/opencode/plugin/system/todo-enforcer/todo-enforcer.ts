/**
 *  Todo Enforcing Plugin — Passive continuation
 *
 ...
 *
 * Defers to autowork: if an autowork loop is active for the session,
...
function wrapSystemPrompt(text: string): string {
	return `<aether_system type="passive-continuation" source="todo-enforcing">\n${text}\n</aether_system>`
}
...
const TodoEnforcingPlugin: Plugin = async ({ client }) => {
	...
	const log: LogFn = (level, message) => {
		try {
			client.app.log({
				body: { service: "todo-enforcing", level, message },
			}).catch(() => {})
		} catch {}
	}
...
export default TodoEnforcingPlugin
