...
interface AvatarProps extends ViewProps {
  ...
  /** Use Aether symbol instead of icon (for SUNA agent) */
  useAetherSymbol?: boolean;
 ...
 * - Aether symbol: 50% of container (larger for brand recognition)
 * - Border: 1.5px solid
...
export function Avatar({
  ...
  const symbolSize = Math.round(size * 0.55); // 55% for Aether symbol (more prominent)
  ...
  const getDefaultColors = () => {
    ...
    // Aether symbol always uses solid black bg with white icon
    if (useAetherSymbol) {
      return {
        bg: '#000000',
        icon: '#FFFFFF',
    ...
    <View 
      ...
      style={[
        { 
          ...
          borderWidth: useAetherSymbol ? 0 : (showBorder ? 1.5 : 0),
