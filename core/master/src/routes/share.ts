 ...
 * /s/{token}/* on master, which validates the token before proxying.
 *
 * Defaults:
...
/**
 * Build the public base URL that reaches port 8000 (master) from outside.
...
export function getMasterPublicBaseUrl(): string {
  ...
  const proxyDomain = process.env.JUSTAVPS_PROXY_DOMAIN || 'aether.cloud'
  ...
  // Cloud without JustAVPS: viaapi proxy
  const sandboxId = process.env.SANDBOX_ID || ''
  const aetherApiUrl = (process.env.AETHER_API_URL || '').replace(/\/v1\/router\/?$/, '')
  if (envMode === 'cloud' && sandboxId && aetherApiUrl) {
    return `${aetherApiUrl}/v1/p/${sandboxId}/8000`
  }
