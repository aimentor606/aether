...
/**
 * Derive the v1 API base URL from AETHER_API_URL.
 * AETHER_API_URL may contain a path suffix (e.g. "/v1/router") used by other
 * subsystems. We only need the origin (scheme + host + port) for direct API calls.
 */
function getApiV1(): string {
  try {
    return new URL(config.AETHER_API_URL).origin + '/v1'
  } catch {
    // Fallback: strip any path and append /v1
    return config.AETHER_API_URL.replace(/\/+$/, '').replace(/\/v1\/.*$/, '') + '/v1'
  }
}

/**
 * Build X-Pipedream-* headers from sandbox env vars.
 * These let the sandbox send its own Pipedream credentials toapi,
...
/**
 * Guard: ensure AETHER_TOKEN is available before proxying toapi.
...
pipedreamRouter.use('*', async (c, next) => {
  if (!config.AETHER_TOKEN) {
    console.error('[Pipedream] AETHER_TOKEN is not set — cannot authenticate toapi.')
    return c.json({
      error: 'Pipedream unavailable: AETHER_TOKEN is not configured. Restart the sandbox.',
    }, 503)
  }

  // Local PIPEDREAM_* env vars are optional overrides.
  // If present, getPipedreamHeaders() sends them as X-Pipedream-* headers.
  // If absent,api uses its own defaults (env or per-account DB creds).
  await next()
})

/**
 * POST /api/pipedream/token
 * Proxy toapi: POST /v1/pipedream/token
...
pipedreamRouter.post('/token',
  describeRoute({
    ...
    description: 'Proxies toapi to fetch an OAuth token for a connected third-party integration. Used by agent tools.',
    responses: {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/pipedream/token`, {
        ...
        headers: {
          ...
          Authorization: `Bearer ${config.AETHER_TOKEN}`,
          ...getPipedreamHeaders(),
        },
...
pipedreamRouter.get('/credentials', async (c) => {
  try {
    const apiUrl = getApiV1()
    const res = await fetch(`${apiUrl}/pipedream/credentials`, {
      headers: { Authorization: `Bearer ${config.AETHER_TOKEN}` },
      signal: AbortSignal.timeout(15_000),
    })
    const data = await res.json()
    if (!res.ok) return c.json(data, res.status as any)
...
export async function pushPipedreamCredsToApi(): Promise<void> {
  ...
  if (!config.AETHER_TOKEN || !config.AETHER_API_URL) {
    console.log('[Pipedream] No AETHER_TOKEN/API_URL — skipping cred push')
    return
  }
