...
interface AvatarProps extends ViewProps {
  ...
  /** Use Acme symbol instead of icon (for SUNA agent) */
  useAcmeSymbol?: boolean;
 ...
 * - Acme symbol: 50% of container (larger for brand recognition)
 * - Border: 1.5px solid
...
export function Avatar({
  ...
  const symbolSize = Math.round(size * 0.55); // 55% for Acme symbol (more prominent)
  ...
  const getDefaultColors = () => {
    ...
    // Acme symbol always uses solid black bg with white icon
    if (useAcmeSymbol) {
      return {
        bg: '#000000',
        icon: '#FFFFFF',
    ...
    <View 
      ...
      style={[
        { 
          ...
          borderWidth: useAcmeSymbol ? 0 : (showBorder ? 1.5 : 0),
