...
import { ensureGlobalMemoryFiles, renderMergedMemoryContext, renderProjectContext, resolveDir } from "./lib/paths"
import { MEMORY_CONTEXT_MARKER, upsertMemoryContextAtPromptEnd, wrapInSystemTags } from "./lib/message-transform"
...
function projectPathForSession(sessionID: string): string | null {
	...
	try {
		const db = new Database(`${resolveDir(import.meta.dir)}/aether.db`, { readonly: true })
		try {
			const row = db
...
export const SessionsPlugin: Plugin = async ({ client, directory }) => {
	...
	try {
		...
		console.error(`[sessions] memory file init failed (non-fatal): ${err}`)
	}
		...
		hooks: {
			"event": async ({ event }: { event: any }) => {
				try {
					...
					console.error(`[sessions] event hook failed: ${err}`)
				}
			},
			"experimental.chat.messages.transform": async (_input: any, output: { messages: any[] }) => {
				try {
					...
					if (currentSessionId) {
						...
						parts.push(wrapInSystemTags(sessionCtx, { type: "session-context", source: "sessions" }))
					}
					...
					if (mergedMemory) {
						...
						const memCtx = `<memory>\n${mergedMemory}\n</memory>`
						parts.push(wrapInSystemTags(memCtx, { type: "memory-context", source: "sessions" }))
					}
					if (currentSessionId) {
						...
						if (projectPath) {
							...
							if (projectCtx) {
								...
								parts.push(wrapInSystemTags(ctx, { type: "project-context", source: "sessions" }))
							}
						}
					}
					...
					console.error(`[sessions] messages.transform failed: ${err}`)
				}
			},
		},
		tool: {
