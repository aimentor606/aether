    ...
    if (searchQuery.trim()) {
      ...
      list = list.filter(
        (c) =>
          ...
          getChannelTypeLabel(c.platform || c.channelType!)
            .toLowerCase()
            .includes(q)
      );
    }
        ...
        <View style={{ padding: 60, alignItems: 'center' }}>
          ...
          <Text
            ...
            {searchQuery
              ...
              : 'Connect messaging platforms to receive and respond to messages.'}
          </Text>
        </View>
      ) : (
        <FlatList
          data={filteredChannels}
          keyExtractor={(item: any) => item.channelConfigId ?? ''}
  ...
  return (
    <Pressable
      ...
      <View
        ...
        <Ionicons
          name={getChannelIcon(channel.platform || channel.channelType!) as any}
          size={18}
          color={muted}
        />
      </View>
      <View style={{ flex: 1 }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 2 }}>
          <Text style={{ fontSize: 11, fontFamily: 'Roobert', color: muted }}>
            {getChannelTypeLabel(channel.platform || channel.channelType!)}
          </Text>
          <Text style={{ fontSize: 11, color: muted }}>·</Text>
          <Text style={{ fontSize: 11, fontFamily: 'Roobert', color: muted }}>
            {channel.sandbox?.name || 'No instance'}
          </Text>
        </View>
      </View>
...
function ChannelStatusDot({ enabled, isDark }: { enabled: boolean; isDark: boolean }) {
  ...
  return (
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
      ...
      <Text style={{ fontSize: 11, fontFamily: 'Roobert', color }}>
...
function ChannelDetailSheet({
  ...
  const createdDate = channel
    ? new Date(channel.created_at || channel.createdAt!).toLocaleDateString('en-US', {
        year: 'numeric',
        month: 'short',
        day: 'numeric',
      })
    : '';
  const updatedDate = channel
    ? new Date(channel.updated_at || channel.updatedAt!).toLocaleDateString('en-US', {
        year: 'numeric',
        month: 'short',
        day: 'numeric',
      })
      ...
      {channel ? (
        <ScrollView
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 20 }}>
            ...
            <View style={{ flex: 1 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                <Text
                  style={{ fontSize: 18, fontFamily: 'Roobert-Semibold', color: fg }}
                  numberOfLines={1}>
                  {channel.name}
                </Text>
                <View
                  ...
                  <Text
                    style={{
                      ...
                      color: channel.enabled ? '#34d399' : muted,
                    }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', padding: 14 }}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={{ fontSize: 11, fontFamily: 'Roobert', color: muted, marginTop: 2 }}>
                  Channels stay attached to the instance they were created for.
                </Text>
              </View>
            </View>
          </View>
          ...
          <View
            ...
            <DetailRow
              label="Type"
              value={getChannelTypeLabel(channel.platform || channel.channelType!)}
              isDark={isDark}
              fg={fg}
              muted={muted}
            />
            ...
            {(channel.default_agent || channel.agentName) && (
              <DetailRow
                label="Agent"
                value={(channel.default_agent || channel.agentName)!}
                isDark={isDark}
                fg={fg}
                muted={muted}
              />
          ...
          <View
            ...
            <View
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: 14, fontFamily: 'Roobert-Medium', color: '#ef4444' }}>
                  Delete Channel
                </Text>
                <Text
                  style={{
              ...
              <Pressable
                ...
                <Text style={{ fontSize: 13, fontFamily: 'Roobert-Medium', color: '#ef4444' }}>
                  Delete
                </Text>
              </Pressable>
            </View>
  ...
  return (
    <View
      ...
      <Text
        style={{
          fontSize: 13,
          fontFamily: mono ? 'monospace' : 'Roobert-Medium',
          color: fg,
          maxWidth: '60%',
        }}
        numberOfLines={1}>
...
function AddChannelSheet({
  ...
  const title =
    view === 'telegram-wizard'
      ...
      : view === 'slack-wizard'
        ? 'Connect Slack'
        : view === 'generic-config' && selectedType
          ? `New ${getChannelTypeLabel(selectedType)} Channel`
          : 'Add Channel';

  const subtitle =
    view === 'telegram-wizard'
      ? 'Set up a Telegram bot for your instance'
      : view === 'slack-wizard'
        ? 'Set up a Slack bot for your instance'
        : view === 'generic-config'
          ? 'Configure your new channel'
      ...
      <BottomSheetScrollView
            ...
            {ALL_CHANNEL_TYPES.map((ct) => {
              ...
              return (
                <Pressable
                  ...
                  <Ionicons
                    name={getChannelIcon(ct.type) as any}
                    size={24}
                    color={isSupported ? fg : muted}
                  />
                  <Text
                    style={{
                      fontSize: 13,
                  ...
                  {!isSupported && (
                    <View
                      ...
                      <Text style={{ fontSize: 10, fontFamily: 'Roobert', color: muted }}>
                        Coming Soon
                      </Text>
                    </View>
                  )}
        ...
        {view === 'generic-config' && (
          <>
            ...
            <Text
              style={{ fontSize: 13, fontFamily: 'Roobert-Medium', color: muted, marginBottom: 6 }}>
              Channel Name
            </Text>
            <BottomSheetTextInput
              value={channelName}
      ...
      {botInfo && (
        <View
          style={{
            ...
            borderRadius: 12,
            borderWidth: 1,
            borderColor: isDark ? 'rgba(190,24,93,0.15)' : 'rgba(190,24,93,0.1)',
            backgroundColor: isDark ? 'rgba(190,24,93,0.05)' : 'rgba(190,24,93,0.03)',
            marginBottom: 16,
          }}>
          <View
            style={{
              ...
              height: 22,
              borderRadius: 11,
              backgroundColor: isDark ? 'rgba(190,24,93,0.15)' : 'rgba(190,24,93,0.1)',
              alignItems: 'center',
              justifyContent: 'center',
            }}>
          ...
          <View
            style={{
              ...
              paddingVertical: 2,
              borderRadius: 6,
              backgroundColor: isDark ? 'rgba(190,24,93,0.1)' : 'rgba(190,24,93,0.06)',
            }}>
            <Text style={{ fontSize: 10, fontFamily: 'Roobert-Medium', color: theme.primary }}>
              Verified
            </Text>
          </View>
        </View>
                  ...
                  <Pressable
                    ...
                    <Text
                      style={{
                        ...
                        color: active ? theme.primaryForeground : muted,
                      }}>
                      {agent.name}
                    </Text>
                  </Pressable>
            ...
            {verifyMutation.isPending ? (
              ...
              <Text
                style={{
                  fontSize: 14,
                  fontFamily: 'Roobert-Medium',
                  color: !botToken.trim() ? muted : theme.primaryForeground,
                }}>
                Verify Token
              </Text>
            )}
...
function SlackWizard({
  ...
  return (
    <>
      <StepIndicator
        steps={['Configure', 'Create App', 'Connect']}
        current={step}
        theme={theme}
        fg={fg}
        muted={muted}
      ...
      {step === 2 && (
        <>
          <View
            ...
            <Text
              style={{ fontSize: 13, fontFamily: 'Roobert-Medium', color: fg, marginBottom: 8 }}>
          ...
          <View
            ...
            <ScrollView horizontal={false} showsVerticalScrollIndicator>
              <Text style={{ fontSize: 11, fontFamily: 'monospace', color: muted }} selectable>
                {manifestJson}
              </Text>
            </ScrollView>
          </View>
          <Pressable
            onPress={handleCopyManifest}
            style={{
              flexDirection: 'row',
              alignItems: 'center',
              justifyContent: 'center',
        ...
        <>
          <View
            ...
            <Text style={{ fontSize: 12, fontFamily: 'Roobert', color: muted, lineHeight: 18 }}>
              Go to your app on api.slack.com → OAuth & Permissions to find the Bot Token, and Basic
              ...
              {connectMutation.isPending ? (
                ...
                <Text
                  style={{
                    ...
                    fontFamily: 'Roobert-Medium',
                    color:
                      !botToken.trim() || !signingSecret.trim() ? muted : theme.primaryForeground,
                  }}>
                  Connect Bot
                </Text>
              )}
                ...
                {isComplete ? (
                  ...
                  <Text
                    style={{
                      fontSize: 11,
                      fontFamily: 'Roobert-Medium',
                      color: isActive ? theme.primaryForeground : muted,
                    }}>
                    {stepNum}
                  </Text>
                )}
            ...
            {i < steps.length - 1 && (
              <View
                style={{
                  width: 20,
                  height: 1,
                  backgroundColor: isComplete ? `${theme.primary}66` : 'rgba(128,128,128,0.15)',
                  marginHorizontal: 6,
                }}
              />
            )}
