 ...
 *
 * Fetches from aether-master's /acme/projects API through the currently
...
export const aetherKeys = {
  projects: () => ['aether', 'projects'] as const,
  project: (id: string) => ['aether', 'projects', id] as const,
};
...
export function useAetherProjects() {
  ...
  return useQuery<AetherProject[]>({
    ...
    queryFn: () => aetherFetch<AetherProject[]>(serverUrl, ''),
...
export function useAetherProject(id: string) {
  ...
  return useQuery<AetherProjectDetail>({
    queryKey: [
      ...aetherKeys.project(id),
      user?.id ?? 'anonymous',
      serverUrl,
      serverVersion,
    ],
    queryFn: () =>
      aetherFetch<AetherProjectDetail>(serverUrl, `/${encodeURIComponent(id)}`),
...
export function useAetherProjectSessions(projectId: string) {
  ...
  return useQuery<any[]>({
    ...
    queryFn: () =>
      aetherFetch<any[]>(
        serverUrl,
        `/${encodeURIComponent(projectId)}/sessions`,
      ),
...
export function useUpdateProject() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      aetherFetch<AetherProject>(serverUrl, `/${encodeURIComponent(id)}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data),
      }),
    onSuccess: (_, vars) => {
      qc.invalidateQueries({ queryKey: aetherKeys.project(vars.id) });
      qc.invalidateQueries({ queryKey: aetherKeys.projects() });
    },
  });
}
...
export function useDeleteProject() {
  ...
  return useMutation({
    mutationFn: (id: string) =>
      aetherFetch<{ deleted: boolean; name: string; path: string }>(
        serverUrl,
