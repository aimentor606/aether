...
export class DaytonaProvider implements SandboxProvider {
  ...
  async create(opts: CreateSandboxOpts): Promise<ProvisionResult> {
    ...
    if (!snapshot) {
      throw new Error('DAYTONA_SNAPSHOT is not configured — set it to the snapshot name (e.g. aether-sandbox-v0.4.1)');
    }

    const daytona = getDaytona();

    // Use AETHER_TOKEN as INTERNAL_SERVICE_KEY — one key for both directions.
    // AETHER_TOKEN (sandbox → api) is already in opts.envVars.
    ...
    // AETHER_URL may be "https://new-api.aether.dev/v1/router" but the
    // sandbox expects the base: "https://new-api.aether.dev".
    ...
    const daytonaSandbox = await daytona.create(
      {
        ...
        envVars: {
          ...
          // Route tool SDK traffic through the Aether router proxy for billing/key injection.
    ...
    const apiBase = config.AETHER_URL.replace(/\/v1\/router\/?$/, '').replace(/\/v1\/?$/, '');
