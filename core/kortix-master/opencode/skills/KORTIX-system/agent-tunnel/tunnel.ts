 ...
 * standalone in the sandbox without @aether/agent-tunnel in node_modules.
 *
  ...
  async resolveTunnelId(): Promise<string> {
    ...
    throw new TunnelClientError(
      ...
      "No tunnel connection found. The user needs to set up Agent Tunnel first:\n" +
        ...
        '2. Run `npx @aether/agent-tunnel connect` on their local machine'
    );
  }
}
...
function getApiBase(): string {
  const raw = getEnv("AETHER_API_URL") || FALLBACK_API_URL;
  const url = raw.startsWith("http") ? raw : FALLBACK_API_URL;
  // Strip /v1/router suffix — AETHER_API_URL sometimes includes the router path
...
const client = new TunnelClient({
  ...
  token: getEnv("AETHER_TOKEN") || "",
  tunnelId: getEnv("AETHER_TUNNEL_ID"),
});
...
async function status() {
  ...
  out({
    ...
    message: hasOnline
      ? undefined
      : "No tunnel is currently online. Ask the user to run `npx @aether/agent-tunnel connect` on their local machine.",
  });
}
