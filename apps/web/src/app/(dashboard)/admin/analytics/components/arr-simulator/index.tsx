...
export function ARRSimulator({ analyticsSource }: ARRSimulatorProps) {
  ...
  const projections = useMemo((): SimulationMonth[] => {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      const monthlyViews = Math.round(
        weeklyVisitors * 4.33 * Math.pow(1 + visitorGrowth / 100, i),
      );
  ...
  const progressPercent = Math.min(
    100,
    ((finalMonth?.arr || 0) / targetARR) * 100,
  );
  ...
  const { data: signupsByDateData } = useSignupsByDate(
    signupsDateFrom,
    signupsDateTo,
  );

  // Fetch views (unique visitors) from analytics source
  const { data: viewsByDateData } = useViewsByDate(
    signupsDateFrom,
    signupsDateTo,
    analyticsSource,
  );
  ...
  const { data: newPaidByDateData } = useNewPaidByDate(
    signupsDateFrom,
    signupsDateTo,
  );

  // Fetch churn data from Stripe Events
  const { data: churnByDateData } = useChurnByDate(
    signupsDateFrom,
    signupsDateTo,
  );
  ...
  const signupsByWeek = useMemo((): Record<number, number> => {
    ...
    Object.entries(signupsByDateData.signups_by_date).forEach(
      ([dateStr, count]) => {
  ...
  const viewsByWeek = useMemo((): Record<number, number> => {
    ...
    Object.entries(viewsByDateData.views_by_date).forEach(
      ([dateStr, count]) => {
        const date = new Date(dateStr);
        const daysSinceStart = Math.floor(
          (date.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24),
  ...
  const newPaidByWeek = useMemo((): Record<number, number> => {
    ...
    Object.entries(newPaidByDateData.new_paid_by_date).forEach(
      ([dateStr, count]) => {
        const date = new Date(dateStr);
        const daysSinceStart = Math.floor(
          (date.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24),
  ...
  const churnByWeek = useMemo((): Record<number, number> => {
    ...
    Object.entries(churnByDateData.churn_by_date).forEach(
      ([dateStr, count]) => {
        const date = new Date(dateStr);
        const daysSinceStart = Math.floor(
          (date.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24),
  ...
  const { data: arrActualsData, isLoading: actualsLoading } =
    useARRWeeklyActuals();
  ...
  const fieldToOverrideKey = (
    field: keyof WeeklyActual,
  ): keyof FieldOverrides => {
  ...
  const handleInputBlur = (
    ...
    const currentData = getActualData(week, platform) || {
      platform,
      views: 0,
      signups: 0,
      newPaid: 0,
    ...
    const apiData: WeeklyActualData = {
      week_number: week,
      week_start_date: formatDateLocal(new Date(2025, 11, 15 + (week - 1) * 7)),
      platform: platform,
  ...
  const deleteWeekActual = (weekNumber: number, platform: Platform) => {
    ...
    setPendingEdits((prev) => {
      const next = { ...prev };
      const fields: (keyof WeeklyActual)[] = [
        'views',
        'newPaid',
        'subscribers',
        'mrr',
  ...
  const [pendingMonthlyEdits, setPendingMonthlyEdits] = useState<
    Record<string, string>
  >({});
  const [pendingMonthlyOverrides, setPendingMonthlyOverrides] = useState<
    Record<string, boolean>
  >({});
  ...
  const handleMonthlyInputBlur = (
    ...
    // Parse shorthand: "25.5k" → 25500, "1M" → 1000000
    const value = parseShorthand(pendingValue);
    const currentData = getMonthlyActualData(monthIndex, platform) || {
      platform,
      views: 0,
      signups: 0,
      newPaid: 0,
  ...
  const handleToggleMonthlyOverride = (
    monthIndex: number,
    platform: Platform,
    field: keyof MonthlyActual,
  ) => {
  ...
  const isMonthlyFieldOverridden = (
    monthIndex: number,
    platform: Platform,
    field: keyof MonthlyActual,
  ): boolean => {
  ...
  const enableMonthlyOverrideInstantly = (
    monthIndex: number,
    platform: Platform,
    monthName: string,
    field: keyof MonthlyActual,
  ...
  const weeklyChartData = weeklyProjections.map((w) => {
    ...
    const webViews = webOverrides.views
      ? webData?.views || 0
      : viewsByWeek[w.week] || 0;
    const webSignups = webOverrides.signups
      ? webData?.signups || 0
      : signupsByWeek[w.week] || 0;
    const webNewPaid = webOverrides.new_paid
      ? webData?.newPaid || 0
      : newPaidByWeek[w.week] || webData?.newPaid || 0;
    const webSubs = webOverrides.subscribers
      ? webData?.subscribers || 0
      : actualSubsByWeek[w.week] || 0;
  ...
  const monthlyActuals = useMemo(() => {
    ...
    weeklyProjections.forEach((week) => {
      ...
      const webViews = webOverrides.views
        ? webData?.views || 0
        : viewsByWeek[week.week] || 0;
      const webSignups = webOverrides.signups
        ? webData?.signups || 0
        : signupsByWeek[week.week] || 0;
      const webNewPaid = webOverrides.new_paid
        ? webData?.newPaid || 0
        : newPaidByWeek[week.week] || webData?.newPaid || 0;
      const webChurn = webOverrides.churn
        ? webData?.churn || 0
        : churnByWeek[week.week] || 0;
      const webSubs = webOverrides.subscribers
        ? webData?.subscribers || 0
  ...
  const subsAtMonthStart = useMemo((): Record<number, number> => {
    ...
    while (currentDate <= endDate) {
      ...
      if (
        ...
        !(currentDate.getMonth() === 11 && currentDate.getFullYear() === 2025)
      ) {
  ...
  const monthlyChartData = useMemo(() => {
    ...
    return monthNames.map((month, idx) => {
      ...
      const webViews = webOverrides.views
        ? webData?.views || 0
        : metricsByCalendarMonth.views[idx] || 0;
      const webSignups = webOverrides.signups
        ? webData?.signups || 0
        : metricsByCalendarMonth.signups[idx] || 0;
      const webNewPaid = webOverrides.new_paid
        ? webData?.newPaid || 0
        : metricsByCalendarMonth.newPaid[idx] || 0;
      const webChurn = webOverrides.churn
        ? webData?.churn || 0
        : metricsByCalendarMonth.churn[idx] || 0;
      const webSubs = webOverrides.subscribers
        ? webData?.subscribers || 0
        : monthlyActuals[idx]?.subscribers || 0;
      const webMrr = webOverrides.mrr
        ? webData?.mrr || 0
        : monthlyActuals[idx]?.mrr || 0;
      const webArr = webOverrides.arr
        ? webData?.arr || 0
        : monthlyActuals[idx]?.arr || 0;
  ...
  // View state
  const [simulatorView, setSimulatorView] = useState<'monthly' | 'weekly'>(
    'monthly',
  );
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Adjust parameters to model your path to {formatCurrency(targetARR)}{' '}
            ARR by June 2026
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                value={arpu || ''}
                onChange={(e) =>
                  setArpu(e.target.value === '' ? 0 : Number(e.target.value))
                }
                onBlur={saveConfig}
                className="h-9"
              />
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-2xl font-bold text-primary">
              {formatCurrency(finalMonth?.arr || 0)}
            </div>
            <p className="text-xs text-muted-foreground mt-1">
              Projected ARR (Jun 2026)
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <div className="text-2xl font-bold">
              {formatNumber(finalMonth?.totalSubs || 0)}
            </div>
            <p className="text-xs text-muted-foreground mt-1">
              Total Subscribers
            </p>
        ...
        <Card>
          <CardContent className="pt-6">
            <div
              className={cn(
                'text-2xl font-bold',
                gapToTarget > 0 ? 'text-red-500' : 'text-green-500',
              )}
            >
              {gapToTarget > 0
        ...
        <CardContent className="pt-6">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm text-muted-foreground">
              {formatCurrency(projections[0]?.arr || 0)} →{' '}
              {formatCurrency(finalMonth?.arr || 0)}
            </span>
          </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">
                Monthly Breakdown (Goal vs Actual)
              </CardTitle>
              <CardDescription>
                Projected growth from Dec 2025 to Jun 2026 with actual data
                comparison
              </CardDescription>
            </CardHeader>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {monthlyFromWeekly.map((month, idx) => {
                      const actual = monthlyActuals[month.monthIndex] || {
                        views: 0,
                        signups: 0,
                        newPaid: 0,
                        churn: 0,
                      ...
                      const webData = getMonthlyActualData(
                        month.monthIndex,
                        'web',
                      );
                      const appData = getMonthlyActualData(
                        month.monthIndex,
                        'app',
                      );
                      const webOverrides = webData?.overrides || {};
                      ...
                      const webViews = webOverrides.views
                        ? webData?.views || 0
                        : metricsByCalendarMonth.views[month.monthIndex] || 0;
                      const webSignups = webOverrides.signups
                        ? webData?.signups || 0
                        : metricsByCalendarMonth.signups[month.monthIndex] || 0;
                      const webNewPaid = webOverrides.new_paid
                        ? webData?.newPaid || 0
                        : metricsByCalendarMonth.newPaid[month.monthIndex] || 0;
                      const webChurn = webOverrides.churn
                        ? webData?.churn || 0
                        : metricsByCalendarMonth.churn[month.monthIndex] || 0;
                      const webSubs = webOverrides.subscribers
                        ? webData?.subscribers || 0
                        : actual.subscribers;
                      const webMRR = webOverrides.mrr
                      ...
                      const totalSignupConvRate =
                        totalViews > 0 && totalSignups > 0
                          ? (totalSignups / totalViews) * 100
                          : null;
                      const totalPaidConvRate =
                        totalSignups > 0 && totalNewPaid > 0
                          ? (totalNewPaid / totalSignups) * 100
                          : null;
                      const monthStartSubs =
                        subsAtMonthStart[month.monthIndex] || 0;
                      const totalChurnRate =
                        monthStartSubs > 0 && totalChurn > 0
                          ? (totalChurn / monthStartSubs) * 100
                          : null;

                      const platformRows = platforms.map(
                        (platform, platformIdx) => {
                          const monthlyOverride = getMonthlyActualData(
                            month.monthIndex,
                            platform,
                          );
                          ...
                          const autoViews =
                            platform === 'web'
                              ? metricsByCalendarMonth.views[
                                  month.monthIndex
                                ] || 0
                              : 0;
                          const autoSignups =
                            platform === 'web'
                              ? metricsByCalendarMonth.signups[
                                  month.monthIndex
                                ] || 0
                              : 0;
                          const autoNewPaid =
                            platform === 'web'
                              ? metricsByCalendarMonth.newPaid[
                                  month.monthIndex
                                ] || 0
                              : 0;
                          const autoChurn =
                            platform === 'web'
                              ? metricsByCalendarMonth.churn[
                                  month.monthIndex
                                ] || 0
                              : 0;
                          ...
                          const effectiveViews =
                            platform === 'app'
                              ? monthlyOverride?.views || 0
                              : isMonthlyFieldOverridden(
                                    month.monthIndex,
                                    platform,
                                    'views',
                                  )
                                ? monthlyOverride?.views || 0
                                : autoViews;
                          const effectiveSignups =
                            platform === 'app'
                              ? monthlyOverride?.signups || 0
                              : isMonthlyFieldOverridden(
                                    month.monthIndex,
                                    platform,
                                    'signups',
                                ...
                                ? monthlyOverride?.signups || 0
                                : autoSignups;
                          const effectiveNewPaid =
                            platform === 'app'
                              ? monthlyOverride?.newPaid || 0
                              : isMonthlyFieldOverridden(
                                    month.monthIndex,
                                    platform,
                                    'newPaid',
                          ...
                          const prevMonthIdx =
                            idx > 0
                              ? monthlyFromWeekly[idx - 1].monthIndex
                              : -1;
                          const prevEffectiveViews =
                            platform === 'web' && prevMonthIdx >= 0
                              ? isMonthlyFieldOverridden(
                                  prevMonthIdx,
                                  platform,
                                  'views',
                                )
                                ? getMonthlyActualData(prevMonthIdx, platform)
                                    ?.views || 0
                                : metricsByCalendarMonth.views[prevMonthIdx] ||
                                  0
                              : 0;
                          const prevEffectiveSignups =
                            platform === 'web' && prevMonthIdx >= 0
                              ? isMonthlyFieldOverridden(
                                  prevMonthIdx,
                                  platform,
                          ...
                          const viewsGrowth =
                            platform === 'web' &&
                            prevEffectiveViews > 0 &&
                            effectiveViews > 0
                              ? (effectiveViews / prevEffectiveViews - 1) * 100
                              : null;
                          const signupsGrowth =
                            platform === 'web' &&
                            prevEffectiveSignups > 0 &&
                            effectiveSignups > 0
                              ? (effectiveSignups / prevEffectiveSignups - 1) *
                                100
                              : null;
                          const newPaidGrowth =
                            platform === 'web' &&
                            prevEffectiveNewPaid > 0 &&
                            effectiveNewPaid > 0
                              ? (effectiveNewPaid / prevEffectiveNewPaid - 1) *
                                100
                              : null;

                          // Calculate signup conversion rate (signups / views)
                          const signupConvRate =
                            effectiveViews > 0 && effectiveSignups > 0
                              ? (effectiveSignups / effectiveViews) * 100
                              : null;

                          // Calculate new paid conversion rate (new paid / signups)
                          const paidConvRate =
                            effectiveSignups > 0 && effectiveNewPaid > 0
                              ? (effectiveNewPaid / effectiveSignups) * 100
                              : null;
                          ...
                          const churnRate =
                            ...
                            monthStartSubs > 0 &&
                            effectiveChurn > 0
                              ? (effectiveChurn / monthStartSubs) * 100
                              : null;
                          ...
                          const renderEditableMonthlyCell = (
                            ...
                            const isOverridden = isMonthlyFieldOverridden(
                              month.monthIndex,
                              platform,
                              field,
                            );
                            const effectiveValue =
                              platform === 'app'
                                ? monthlyOverride?.[field] || 0
                                : isOverridden
                                  ? monthlyOverride?.[field] || 0
                                  : autoValue;
                            const displayValue = isCurrency
                              ? formatCurrency(effectiveValue as number)
                              : formatNumber(effectiveValue as number);
                            const meetsGoal =
                              (effectiveValue as number) >= goalValue;
                            ...
                            return (
                              <td className="text-right p-1">
                                <div className="flex items-center justify-end gap-1">
                                  ...
                                  {isOverridden ||
                                  `${month.monthIndex}-${platform}-${fieldToOverrideKey(field as keyof WeeklyActual)}` in
                                    pendingMonthlyEdits ? (
                                    <Input
                                      type="text"
                                      value={getMonthlyInputValue(
                                    ...
                                    <button
                                      onClick={() =>
                                        enableMonthlyOverrideInstantly(
                                          month.monthIndex,
                                          platform,
                                          month.month,
                                          field,
                          ...
                          return (
                            <tr
                              key={`${month.month}-${platform}`}
                              className={cn(
                                'border-b',
                                isLastMonth ? 'bg-primary/5 font-medium' : '',
                              )}
                            >
                              <td className="p-3">
                                <div className="flex items-center gap-2">
                      ...
                      const totalRow = (
                        ...
                        >
                          ...
                          <td className="text-right p-2 font-medium">
                            {totalSignupConvRate !== null
                              ? `${totalSignupConvRate.toFixed(1)}%`
                              : '—'}
                          </td>
                          {/* New Paid Goal */}
                          <td className="text-right p-2 text-muted-foreground">
                            {formatNumber(month.newPaid)}
                          </td>
                          {/* New Paid Actual - total */}
                          <td className="text-right p-2 font-semibold">
                            {totalNewPaid > 0
                              ? formatNumber(totalNewPaid)
                              : '—'}
                          </td>
                          {/* New Paid Growth */}
                          <td className="text-right p-2 text-muted-foreground">
                          ...
                          {/* Paid Conv Rate */}
                          <td className="text-right p-2 font-medium">
                            {totalPaidConvRate !== null
                              ? `${totalPaidConvRate.toFixed(1)}%`
                              : '—'}
                          </td>
                          {/* Churn - total */}
                          <td className="text-right p-2 font-semibold">
                            {totalChurn > 0 ? formatNumber(totalChurn) : '—'}
                          </td>
                          {/* Churn Rate */}
                          <td
                            className={cn(
                              'text-right p-2 font-medium',
                              totalChurnRate !== null && totalChurnRate > 0
                                ? 'text-red-500'
                                : 'text-muted-foreground',
                            )}
                          >
                            {totalChurnRate !== null
                              ? `${totalChurnRate.toFixed(1)}%`
                              : '—'}
                          </td>
                          {/* Total Subs Goal */}
                          <td className="text-right p-2 text-muted-foreground">
                          ...
                          <td className="text-right p-2 font-semibold">
                            {totalSubs > 0 ? formatNumber(totalSubs) : '—'}
                          </td>
                          {/* MRR Goal */}
                          <td className="text-right p-2 text-muted-foreground">
                          ...
                          {/* ARR Actual - total */}
                          <td className="text-right p-2 font-semibold">
                            {totalARR > 0 ? formatCurrency(totalARR) : '—'}
                          </td>
                        </tr>
                      );
      ...
      {simulatorView === 'weekly' && (
        <>
                ...
                <div className="h-[250px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={weeklyChartData}>
                      ...
                      <Tooltip
                        formatter={
                          ((v: number) => [`$${v.toLocaleString()}`, '']) as any
                        }
                      />
                      <Legend />
                      <Line
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">
                📊 Weekly Tracking (Dec 2025 - Jun 2026)
              </CardTitle>
              <CardDescription>
                Data is auto-synced from Stripe/Vercel. Edit any value to
                override it — once edited, it stays locked (🔒) and won't be
                overwritten by auto-sync. Click the lock icon to unlock and
                resume syncing.
              </CardDescription>
            </CardHeader>
            <CardContent className="p-0">
                ...
                <table className="w-full text-xs">
                  <thead className="sticky top-0 bg-background">
                    <tr className="border-b bg-muted/50">
                      ...
                      <th className="text-center p-2 font-medium" colSpan={2}>
                        Views
                      </th>
                      ...
                      <th className="text-center p-2 font-medium" colSpan={3}>
                        Subscribers
                      </th>
                      <th className="text-center p-2 font-medium" colSpan={3}>
                        MRR
                    ...
                    <tr className="border-b bg-muted/30">
                      <th></th>
                      <th></th>
                      <th className="text-right p-2 text-muted-foreground font-normal text-[10px]">
                        Goal
                      </th>
                      <th className="text-right p-2 text-muted-foreground font-normal text-[10px]">
                        Actual
                  ...
                  <tbody>
                    {weeklyProjections.map((week) => {
                      ...
                      const webNewPaid = webOverrides.new_paid
                        ? webActual?.newPaid || 0
                        : (newPaidByWeek[week.week] ?? webActual?.newPaid ?? 0);
                      const webChurn = webOverrides.churn
                        ? webActual?.churn || 0
                        : churnByWeek[week.week] || 0;
                      const webSubs = webOverrides.subscribers
                        ? webActual?.subscribers || 0
                        : (actualSubsByWeek[week.week] ?? 0);
                      ...
                      // Total variances
                      const totalSubsVar = getVariance(
                        totalSubs,
                        week.subscribers,
                      );
                      ...
                      const platformRows = platforms.map(
                        (platform, platformIdx) => {
                          ...
                          const viewsOverridden = isFieldOverridden(
                            week.week,
                            platform,
                            'views',
                          );
                          const signupsOverridden = isFieldOverridden(
                            week.week,
                            platform,
                            'signups',
                          );
                          const newPaidOverridden = isFieldOverridden(
                            week.week,
                            platform,
                            'new_paid',
                          );
                          const subscribersOverridden = isFieldOverridden(
                            week.week,
                            platform,
                            'subscribers',
                          );
                          ...
                          const churnOverridden = isFieldOverridden(
                            week.week,
                            platform,
                            'churn',
                          );
                          const autoChurn =
                            platform === 'web'
                              ? churnByWeek[week.week] || 0
                              : 0;
                          const effectiveChurn = churnOverridden
                            ? actual?.churn || 0
                            : autoChurn;
                          ...
                          const prevActual = getActualData(
                            prevWeekNum,
                            platform,
                          );
                          const prevViewsOverridden =
                            prevActual?.overrides?.views || false;
                          const prevSignupsOverridden =
                            prevActual?.overrides?.signups || false;
                          const prevNewPaidOverridden =
                            prevActual?.overrides?.new_paid || false;

                          // For Week 1, use week0Baseline (only for web); otherwise use fetched data or overridden data
                          const prevAutoViews =
                            prevWeekNum === 0 && platform === 'web'
                              ? week0Baseline.views
                              : prevViewsOverridden
                                ? prevActual?.views || 0
                                : platform === 'web'
                                  ? (viewsByWeek[prevWeekNum] ?? 0)
                                  : 0;
                          const prevAutoSignups =
                            prevWeekNum === 0 && platform === 'web'
                              ? week0Baseline.signups
                              : prevSignupsOverridden
                                ? prevActual?.signups || 0
                                : platform === 'web'
                                  ? (signupsByWeek[prevWeekNum] ?? 0)
                                  : 0;
                          const prevAutoNewPaid =
                            prevWeekNum === 0 && platform === 'web'
                              ? week0Baseline.newPaid
                              : prevNewPaidOverridden
                                ? prevActual?.newPaid || 0
                                : platform === 'web'
                                  ? (newPaidByWeek[prevWeekNum] ?? 0)
                                  : 0;

                          // Calculate growth rates (week-over-week) using effective values
                          const viewsGrowth =
                            prevAutoViews > 0 && effectiveViews > 0
                              ? (effectiveViews / prevAutoViews - 1) * 100
                              : null;
                          const signupsGrowth =
                            prevAutoSignups > 0 && effectiveSignups > 0
                              ? (effectiveSignups / prevAutoSignups - 1) * 100
                              : null;
                          const newPaidGrowth =
                            prevAutoNewPaid > 0 && effectiveNewPaid > 0
                              ? (effectiveNewPaid / prevAutoNewPaid - 1) * 100
                              : null;
                          ...
                          return (
                            <tr
                              key={`${week.week}_${platform}`}
                              className={`border-b hover:bg-muted/30 ${week.week === weeklyProjections.length ? 'bg-primary/5 font-medium' : ''} ${platform === 'app' ? 'bg-purple-50/50 dark:bg-purple-950/20' : ''}`}
                            >
                                ...
                                <div className="flex items-center justify-end gap-1">
                                  {(viewsOverridden || platform === 'app') && (
                                    <button
                                      ...
                                      title={
                                        platform === 'app'
                                          ? 'App data (manual entry)'
                                          : 'Locked (manual override). Click to unlock and sync from Vercel.'
                                      }
                                    >
                                      <Lock className="h-3 w-3" />
                                    </button>
                              ...
                              <td
                                className={cn(
                                  'text-right p-1 text-[10px] font-medium',
                                  getGrowthColor(viewsGrowth),
                                )}
                              >
                                {formatGrowth(viewsGrowth)}
                                  ...
                                  {(signupsOverridden ||
                                    ...
                                    <button
                                      ...
                                      title={
                                        platform === 'app'
                                          ? 'App data (manual entry)'
                                          : 'Locked (manual override). Click to unlock and sync from database.'
                                      }
                                    >
                                      <Lock className="h-3 w-3" />
                                    </button>
                                ...
                                <div className="flex items-center justify-end gap-1">
                                  {(newPaidOverridden ||
                                    ...
                                    <button
                                      ...
                                      title={
                                        platform === 'app'
                                          ? 'App data (manual entry from RevenueCat)'
                                          : 'Locked (manual override). Click to unlock and sync from Stripe.'
                                      }
                                    >
                                      <Lock className="h-3 w-3" />
                                    </button>
                              ...
                              {(() => {
                                ...
                                const monthStartSubs =
                                  subsAtMonthStart[monthIdx] || 0;
                                const churnRate =
                                  monthStartSubs > 0 && weekChurn > 0
                              ...
                              <td className="text-right p-1">
                                <div className="flex items-center justify-end gap-1">
                                  {(subscribersOverridden ||
                                    ...
                                    <button
                                      ...
                                      title={
                                        platform === 'app'
                                          ? 'App data (manual entry)'
                                          : 'Locked (manual override). Click to unlock and use calculated value.'
                                      }
                                    >
                                      <Lock className="h-3 w-3" />
                                    </button>
                                  ...
                                  platform === 'app' ? (
                                    ...
                                    >
                                      {effectiveSubs > 0
                                        ? formatNumber(effectiveSubs)
                                        : '—'}
                                    </button>
                                  )}
                                </div>
                              </td>
                              <td className="text-right p-1 text-[10px] text-muted-foreground">
                                —
                              </td>
                              {/* MRR */}
                              <td className="text-right p-1"></td>
                                  ...
                                  {(isFieldOverridden(
                                    ...
                                    <button
                                      ...
                                      title={
                                        platform === 'app'
                                          ? 'App data (manual entry)'
                                          : 'Locked (manual override). Click to unlock.'
                                      }
                                    >
                                      <Lock className="h-3 w-3" />
                                    </button>
                              ...
                              <td className="p-1">
                                {(actual?.newPaid ||
                                  ...
                                  <button
                                    onClick={() =>
                                      deleteWeekActual(week.week, platform)
                                    }
                      ...
                      const totalRow = (
                        <tr
                          key={`${week.week}_total`}
                          className={cn(
                            'border-b bg-muted/30 font-medium',
                            week.week === weeklyProjections.length
                              ? 'bg-primary/10'
                              : '',
                          )}
                        >
                          ...
                          <td className="text-right p-1 text-muted-foreground">
                            {formatNumber(week.visitors)}
                          </td>
                          {/* Views Actual - total */}
                          <td className="text-right p-1 font-semibold text-[10px]">
                            {totalViews > 0 ? formatNumber(totalViews) : '—'}
                          </td>
                          {/* Growth - skip */}
                          <td className="text-right p-1 text-muted-foreground text-[10px]">
                            —
                          </td>
                          {/* Signups Goal */}
                          <td className="text-right p-1">
                            {formatNumber(week.signups)}
                          </td>
                          {/* Signups Actual - total */}
                          <td className="text-right p-1 font-semibold text-[10px]">
                            {totalSignups > 0
                          ...
                          <td
                            className={cn(
                              'text-right p-1 text-[10px]',
                              totalSubsVar.color,
                            )}
                          >
                            {totalSubs > 0
                              ? `${totalSubsVar.value >= 0 ? '+' : ''}${totalSubsVar.value.toFixed(1)}%`
                              : '—'}
                          </td>
                          {/* MRR Goal */}
                          <td className="text-right p-1">
                            {formatCurrency(week.mrr)}
                          </td>
                          {/* MRR Actual - total */}
                          <td className="text-right p-1 font-semibold text-[10px]">
                            {totalMRR > 0 ? toShorthand(totalMRR) : '—'}
                          </td>
                          {/* MRR Var */}
                          <td
                            className={cn(
                          ...
                          >
                            {totalMRR > 0
                              ? `${totalMrrVar.value >= 0 ? '+' : ''}${totalMrrVar.value.toFixed(1)}%`
                              : '—'}
                          </td>
                          {/* ARR Goal */}
                          <td className="text-right p-1 font-medium">
                            {formatCurrency(week.arr)}
                          </td>
                          {/* ARR Actual - total */}
                          <td className="text-right p-1 font-semibold text-[10px]">
                            {totalARR > 0 ? toShorthand(totalARR) : '—'}
                          </td>
                          {/* ARR Var */}
                          <td
                            className={cn(
                          ...
                          >
                            {totalARR > 0
                              ? `${totalArrVar.value >= 0 ? '+' : ''}${totalArrVar.value.toFixed(1)}%`
                              : '—'}
                          </td>
                          {/* Delete - empty */}
                          <td className="p-1"></td>
