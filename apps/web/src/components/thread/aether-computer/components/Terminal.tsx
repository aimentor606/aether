...
const TERMINAL_HISTORY_KEY = 'aether-terminal-history';
...
export const Terminal = memo(function Terminal({
  ...
  const [lines, setLines] = useState<TerminalLine[]>([
    { id: '0', type: 'output', content: 'Welcome to Aether Terminal' },
    {
      id: '1',
      type: 'output',
      content: 'Type commands below and press Enter to execute.',
    },
  ...
  const addLine = useCallback(
    (type: TerminalLine['type'], content: string, lineCwd?: string) => {
    ...
    async (command: string) => {
      ...
      if (command.trim().startsWith('cd ')) {
        ...
        setCommandHistory((prev) => {
          const updated = [...prev.filter((c) => c !== command), command].slice(
            -100,
          );
      ...
      try {
        ...
        if (!response.success || response.error) {
          addLine(
            'error',
            `Error: ${response.error?.message || 'Command execution failed'}`,
          );
  ...
  return (
    ...
    >
      <div className="flex items-center h-7 px-3 bg-zinc-200/80 dark:bg-[#24283b] border-b border-zinc-300 dark:border-[#414868] gap-2 flex-shrink-0">
        ...
        {isExecuting && <AetherLoader size="small" className="ml-auto" />}
      </div>
      ...
      >
        {lines.map((line) => (
          <div key={line.id} className="flex items-start">
            {line.type === 'input' ? (
              <>
                <span className="text-blue-500 dark:text-[#7aa2f7] mr-1.5">
                  {getPromptDisplay(line.cwd || '/workspace')}
                </span>
                <span className="text-emerald-600 dark:text-[#9ece6a] mr-2">
                  ❯
                </span>
                <span className="text-zinc-900 dark:text-[#c0caf5]">
                  {line.content}
                </span>
              </>
            ) : (
