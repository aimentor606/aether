...
export function useAetherTasks(projectId?: string, status?: string) {
  ...
  return useQuery({
    ...
    queryFn: () => acmeTaskFetch<AetherTask[]>(serverUrl, qs),
    enabled: !!projectId,
...
export function useAetherTask(id: string) {
  ...
  return useQuery({
    ...
    queryFn: () => acmeTaskFetch<AetherTask>(serverUrl, `/${encodeURIComponent(id)}`),
    enabled: !!id,
  });
}
...
export function useUpdateAetherTask() {
  ...
  return useMutation({
    mutationFn: ({ id, ...data }: { id: string } & Partial<AetherTask>) =>
      acmeTaskFetch<AetherTask>(serverUrl, `/${encodeURIComponent(id)}`, {
        method: 'PATCH',
