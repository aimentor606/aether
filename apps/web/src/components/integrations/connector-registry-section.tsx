...
const SOURCE_CONFIG: Record<
  string,
  { icon: typeof Plug; label: string; color: string }
> = {
...
function ConnectorCard({
  ...
  const liveConnection = pipedreamConnections.find(
    (c) => c.app === slug || c.app === connector.name,
  );
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 px-4 py-3">
        <div
          className={cn(
            'flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-border/30 bg-muted/50',
            cfg.color,
          )}
        >
      ...
      {expanded && (
        <div className="px-4 pb-3 pt-0 border-t border-border/30 space-y-1.5">
          {connector.pipedream_slug && (
            <div className="text-xs pt-2">
              <span className="text-muted-foreground">pipedream:</span>{' '}
              <span className="font-mono">{connector.pipedream_slug}</span>
            </div>
          )}
          {connector.env_keys && connector.env_keys.length > 0 && (
          ...
          {isLive && liveConnection && (
            <div className="flex items-center gap-2 pt-1">
              ...
              <span className="text-xs text-emerald-600">
                Live as <span className="font-mono">{liveConnection.app}</span>
              </span>
            </div>
          )}
          <div className="text-[10px] text-muted-foreground/50 pt-1">
            {connector.auto_generated ? 'Auto-created · ' : ''}
...
export function ConnectorRegistrySection({
  ...
  const { data: connectors, isLoading } = useAetherConnectors();
  if (isLoading || !connectors || connectors.length === 0) return null;

  const sorted = [...connectors].sort((a, b) => {
    const aLive = pipedreamConnections.some(
      (c) => c.app === (a.pipedream_slug || a.name),
    );
    const bLive = pipedreamConnections.some(
      (c) => c.app === (b.pipedream_slug || b.name),
    );
  ...
  const liveCount = sorted.filter((c) =>
    pipedreamConnections.some((pc) => pc.app === (c.pipedream_slug || c.name)),
  ).length;
