 ...
 * webhooks are handled directly by master.
 */
...
async function handleChannelCommand(
  ...
  switch (command) {
    ...
    case 'info': {
      ...
      return {
        ...
        text: [
          ...
          `Agent: ${channel.default_agent || 'aether'}`,
    ...
    case 'agent': {
      if (!arg) {
        return { handled: true, text: `Current agent: ${channel.default_agent || 'aether'}\nUsage: ${p}agent <name>` }
      }
  ...
  const modelOverride = channel.default_model
    ? (() => {
        ...
        return parts.length >= 2
          ...
          : { providerID: 'aether', modelID: channel.default_model }
      })()
    : undefined
