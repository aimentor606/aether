...
const AETHER_MARKETPLACE_NAMESPACE = 'aether'
const AETHER_MARKETPLACE_REGISTRY_URL = 'https://aether-registry-6om.pages.dev'

function getWorkspacePaths() {
  const workspaceRoot = process.env.AETHER_WORKSPACE || '/workspace'
  const opencodeDir = path.join(workspaceRoot, '.opencode')
  return {
    workspaceRoot,
    opencodeDir,
    legacyOpencodeDir: path.join(workspaceRoot, '.aether', '.opencode'),
...
async function ensureRegistry(): Promise<void> {
  ...
  const hasKortixRegistry =
    configText.includes('"kortix"') && configText.includes(AETHER_MARKETPLACE_REGISTRY_URL)

  if (!hasKortixRegistry) {
    await runOcx([
      'registry',
      'add',
      AETHER_MARKETPLACE_REGISTRY_URL,
      '--name',
      AETHER_MARKETPLACE_NAMESPACE,
    ])
  }
}
...
async function listInstalledMarketplaceComponents(): Promise<string[]> {
  ...
  return components
    .filter((component) => component.name.startsWith(`${AETHER_MARKETPLACE_NAMESPACE}/`))
...
marketplaceRouter.get('/status', async (c) => {
  try {
    ...
    const registryConfigured =
      ocxConfigText.includes('"kortix"') && ocxConfigText.includes(AETHER_MARKETPLACE_REGISTRY_URL)
    ...
    return c.json({
      ...
      status: {
        ...
        ocxConfigPath,
        opencodeConfigPath,
        registryUrl: AETHER_MARKETPLACE_REGISTRY_URL,
        registryConfigured,
        installedComponents,
      },
    })
...
marketplaceRouter.post('/install', async (c) => {
  try {
    ...
    const namespace = body.namespace || AETHER_MARKETPLACE_NAMESPACE
