/**
 *  Autowork Plugin — Explicit execution loop
 *
 ...
 *
 * Does NOT handle passive todo continuation — that is todo-enforcing.
 *
 * Signals to todo-enforcing which sessions have an active autowork loop
 * so it can defer.
 */
...
try {
	const mod = require("../todo-enforcing/todo-enforcing")
...
function wrapSystemPrompt(text: string, type: string): string {
	return `<aether_system type="${type}" source="autowork">\n${text}\n</aether_system>`
}
...
const AutoworkPlugin: Plugin = async ({ client }) => {
	...
	const log: LogFn = (level, message) => {
		try {
			client.app.log({
				body: { service: "autowork", level, message },
			}).catch(() => {})
		} catch {}
	}
	...
	return {
		...
		"chat.message": async (input: any, output: any) => {
			try {
				...
				// ── /autowork-cancel ──
				if (messageText.includes("AETHER_AUTOWORK_CANCEL") || /\/autowork-cancel\b/.test(messageText)) {
				...
				const autoworkMatch = pending?.command === "autowork"
					...
					|| messageText.includes("AETHER_AUTOWORK")
				...
				//   2. AETHER_AUTOWORK marker in message text (system-injected)
