...
function SessionTabsContainer({ children }: { children: React.ReactNode }) {
  ...
  const pageTabIds = tabOrder.filter((id) => {
    ...
    return (
      t === 'settings' || t === 'page' || t === 'project' || t === 'dashboard'
    );
  });
          ...
          <TabErrorBoundary tabId={id}>
            <Suspense fallback={null}>
              <SessionLayout sessionId={id}>
                <SessionChat
                  sessionId={id}
                  hideHeader={obActive && id === obSessionId}
                />
              </SessionLayout>
            </Suspense>
          </TabErrorBoundary>
  ...
  return (
    <AlertDialog>
      ...
      <AlertDialogContent className="max-w-sm rounded-2xl">
        <AlertDialogHeader>
          <AlertDialogTitle className="text-base font-medium text-foreground/90">
            Skip onboarding?
          </AlertDialogTitle>
          <AlertDialogDescription className="text-[13px] text-muted-foreground/60 leading-relaxed">
            You can set up your profile anytime. Your agent will work fine — it
            just won&apos;t know your preferences yet.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter className="gap-2 sm:gap-2">
          <AlertDialogCancel className="rounded-xl text-[13px]">
            Continue
          </AlertDialogCancel>
          <AlertDialogAction
            onClick={onConfirm}
...
export default function DashboardLayoutContent({
  ...
  useEffect(() => {
    ...
    const check = async () => {
      ...
      if (!instanceUrl) return;
      try {
        const f = await import('@/lib/auth-token').then(
          (m) => m.authenticatedFetch,
        );
        const res = await f(
          `${instanceUrl}/env/ONBOARDING_COMPLETE`,
          undefined,
        ...
        if (!res.ok) {
          ...
          if (res.status === 403 || res.status >= 500) {
            console.warn(
              `[onboarding] /env/ONBOARDING_COMPLETE returned ${res.status} — entering setup mode`,
            );
        ...
        console.warn(
          '[onboarding] Failed to check ONBOARDING_COMPLETE — entering setup mode',
        );
        ob.enter({ skipBoot: true, skipSetup: false });
        setOnboardingChecked(true);
      }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const alreadyFired =
          (await readEnv('ONBOARDING_COMMAND_FIRED')) === 'true';
        ...
        if (existing) {
          try {
            ...
            if (
              !(msgs.data ?? []).some((m: any) => m.info?.role === 'assistant')
            )
              needsCmd = true;
          } catch {
        ...
        if (!sid) {
          const s = await createSessionRef.current.mutateAsync({
            title: 'Aether Onboarding',
          });
        ...
        if (
          /still syncing|provisioning|not ready|sandbox route/i.test(msg) ||
          obRetries.current < 3
        ) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        ...
        if ((msgs.data ?? []).some((m: any) => m.info?.role === 'assistant'))
          return;
        obCmdFired.current = true;
        persistEnv('ONBOARDING_COMMAND_FIRED', 'true');
        void c.session
