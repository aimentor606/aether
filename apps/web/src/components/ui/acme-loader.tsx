...
/**
 * AcmeLoader - A unified circular loading animation component
 * 
 ...
 * <AcmeLoader variant="white" />
 * 
 * // Always black (for light backgrounds in any theme)
 * <AcmeLoader variant="black" />
 * 
 * // Custom size
 * <AcmeLoader size="large" />
 * ```
 */
export function AcmeLoader({
  size = 'medium',
  speed = 1.2,
  customSize,
  className,
      ...
      <div
        style={{
          ...
          animation: autoPlay && loop
            ? `acme-spin ${animationDuration}s linear infinite`
            : autoPlay
              ? `acme-spin ${animationDuration}s linear`
              : 'none',
        }}
      />
    </div>
