...
AETHER_LOCAL_IMAGES="${AETHER_LOCAL_IMAGES:-0}"
AETHER_LOCAL_TAG="${AETHER_LOCAL_TAG:-latest}"
AETHER_BUILD_LOCAL_IMAGES="${AETHER_BUILD_LOCAL_IMAGES:-0}"
AETHER_PULL_PARALLELISM="${AETHER_PULL_PARALLELISM:-4}"
...
AETHER_LOCAL_REPO_ROOT="${AETHER_LOCAL_REPO_ROOT:-$REPO_ROOT}"
...
parse_args() {
  while [ "$#" -gt 0 ]; do
    case "$1" in
      --local)
        AETHER_LOCAL_IMAGES=1
        shift
        ;;
      --build-local)
        AETHER_LOCAL_IMAGES=1
        AETHER_BUILD_LOCAL_IMAGES=1
        shift
        ;;
      --local-tag)
...
if [ -z "$AETHER_VERSION" ] && [ "$AETHER_LOCAL_IMAGES" != "1" ]; then
  AETHER_VERSION=$(resolve_latest_version)
fi
# Ensure AETHER_VERSION always has a value (fallback to 'latest' tag)
AETHER_VERSION="${AETHER_VERSION:-latest}"

IMAGE_TAG="$AETHER_VERSION"
SANDBOX_IMAGE_REPO="aether/computer"
if [ "$AETHER_LOCAL_IMAGES" = "1" ]; then
  IMAGE_TAG="$AETHER_LOCAL_TAG"
  SANDBOX_IMAGE_REPO="aether/computer"
fi
...
[ -n "${AETHER_FRONTEND_IMAGE:-}" ] && FRONTEND_IMAGE_OVERRIDDEN="1"
[ -n "${AETHER_API_IMAGE:-}" ] && API_IMAGE_OVERRIDDEN="1"
[ -n "${AETHER_SANDBOX_IMAGE:-}" ] && SANDBOX_IMAGE_OVERRIDDEN="1"
...
ensure_local_build_requirements() {
  [ -d "$AETHER_LOCAL_REPO_ROOT/apps/web" ] || fatal "Local repo root not found at ${AETHER_LOCAL_REPO_ROOT}."
  [ -x "$PNPM_BIN" ] || fatal "pnpm is required for --build-local."
}

rebuild_local_images() {
  ensure_local_build_requirements
  local build_script="$AETHER_LOCAL_REPO_ROOT/scripts/build-local-images.sh"
  [ -f "$build_script" ] || fatal "Local build script not found: ${build_script}"

  info "Rebuilding local installer images from ${AETHER_LOCAL_REPO_ROOT}"
  echo ""
  bash "$build_script" --tag "$AETHER_LOCAL_TAG"
...
resolve_release_images() {
  [ "$AETHER_LOCAL_IMAGES" = "1" ] && return 0

  dots "Resolving images for v${AETHER_VERSION}"
  ...
  if ! docker_manifest_exists "$FRONTEND_IMAGE"; then
    ...
    if docker_manifest_exists "$fallback_frontend"; then
      ...
      fatal "Frontend image not found for ${AETHER_VERSION}, and latest fallback is unavailable."
    fi
  fi
  ...
  if ! docker_manifest_exists "$API_IMAGE"; then
    ...
    if docker_manifest_exists "$fallback_api"; then
      ...
      fatal "API image not found for ${AETHER_VERSION}, and latest fallback is unavailable."
    fi
  fi
  ...
  if ! docker_manifest_exists "$SANDBOX_IMAGE"; then
    ...
    if docker_manifest_exists "$fallback_sandbox"; then
      ...
      fatal "Sandbox image not found for ${AETHER_VERSION}, and latest fallback is unavailable."
    fi
  fi
...
EOF
  ...
  printf "\n"
  printf "    ${FADED}v${AETHER_VERSION}  ·  One-Click Installer  ·  Local or VPS${NC}\n"
  printf "\n"
}
...
"
    ...
    frontend_supabase_env="      - AETHER_PUBLIC_SUPABASE_URL=\${SUPABASE_PUBLIC_URL}
      - AETHER_PUBLIC_SUPABASE_ANON_KEY=\${SUPABASE_ANON_KEY}
      - AETHER_PUBLIC_BACKEND_URL=\${API_PUBLIC_URL}/v1
      - AETHER_PUBLIC_BILLING_ENABLED=false
      - AETHER_PUBLIC_ENV_MODE=local
      - AETHER_PUBLIC_APP_URL=\${PUBLIC_URL}
      - AETHER_PUBLIC_SANDBOX_ID=\${SANDBOX_CONTAINER_NAME}
    ...
    frontend_supabase_env="      - AETHER_PUBLIC_SUPABASE_URL=\${SUPABASE_URL}
      - AETHER_PUBLIC_SUPABASE_ANON_KEY=\${SUPABASE_ANON_KEY}
      - AETHER_PUBLIC_BACKEND_URL=\${API_PUBLIC_URL}/v1
      - AETHER_PUBLIC_BILLING_ENABLED=false
      - AETHER_PUBLIC_ENV_MODE=local
...
${supabase_db_env}
      ...
      - AETHER_LOCAL_IMAGES=\${AETHER_LOCAL_IMAGES}
      - DOCKER_HOST=unix:///var/run/docker.sock
      - AETHER_URL=http://aether-api:8008/v1/router
      ...
      - SANDBOX_VERSION=\${AETHER_VERSION}
      - TUNNEL_SIGNING_SECRET=\${TUNNEL_SIGNING_SECRET}
      - AETHER_ROUTER_INTERNAL_ENABLED=false
...
AETHER_LOCAL_IMAGES=${AETHER_LOCAL_IMAGES}
AETHER_LOCAL_TAG=${AETHER_LOCAL_TAG}
AETHER_LOCAL_REPO_ROOT=${AETHER_LOCAL_REPO_ROOT}
...
AETHER_SANDBOX_VERSION=${AETHER_VERSION}
ENVEOF
...
ALLOWED_SANDBOX_PROVIDERS=local_docker
AETHER_LOCAL_IMAGES=${AETHER_LOCAL_IMAGES}
AETHER_URL=${API_PUBLIC_URL}/v1/router
...
_free_aether_ports() {
  ...
  docker ps -a --format '{{.Names}}' 2>/dev/null \
    | grep -E "^${project_name}-" \
    | grep -v -E "^(aether-sandbox|${SANDBOX_NAME})$" \
    | xargs -r docker rm -f 2>/dev/null || true
...
_rebuild_local_images() {
  [ -n "$LOCAL_REPO_ROOT" ] || { _err "AETHER_LOCAL_REPO_ROOT is not set"; exit 1; }
...
pull_and_start() {
  ...
  if [ "$AETHER_LOCAL_IMAGES" = "1" ]; then
    if [ "$AETHER_BUILD_LOCAL_IMAGES" = "1" ]; then
      rebuild_local_images
    fi
