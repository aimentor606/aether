...
export type InternalAetherEnv = 'dev' | 'staging' | 'prod';
...
// ─── Env Schema ─────────────────────────────────────────────────────────────
//
// Every env var that aether-api reads is declared here.
// Categories:
...
const envSchema = z.object({
  ...
  JUSTAVPS_WEBHOOK_URL:               optStr,   // URL where JustAVPS should send webhook events (e.g. https://api.aether.dev/v1/platform/webhooks/justavps)
  ...
  // ── Sandbox Platform ──────────────────────────────────────────────────────
  // AETHER_URL is auto-derived from PORT if not explicitly set (see validateEnv).
  AETHER_URL:                  optStr,
  ALLOWED_SANDBOX_PROVIDERS:   optStrDefault('local_docker'),
  SANDBOX_IMAGE:               optStr,  // overridden below if empty
  AETHER_LOCAL_IMAGES:         optBoolFalse,
  DOCKER_HOST:                 optStr,
  ...
  SANDBOX_CONTAINER_NAME:      z.string().optional().transform(v => v || undefined).default('aether-sandbox'),
  ...
  MAILTRAP_FROM_EMAIL:         optStrDefault('noreply@aether.dev'),
  MAILTRAP_FROM_NAME:          optStrDefault('Aether'),
  ...
  AETHER_MASTER_URL:           optStr,
  OPENCODE_URL:                optStr,
  AETHER_DATA_DIR:             optStr,
});
...
function validateEnv(): z.infer<typeof envSchema> {
  ...
  const billingWillBeEnabled = (raw as any).AETHER_BILLING_INTERNAL_ENABLED === 'true' || (raw as any).AETHER_BILLING_INTERNAL_ENABLED === true || (raw as any).ENV_MODE === 'cloud';
  ...
  // ── Conditional: AETHER_URL — required for sandbox routing ──────────────
  ...
  if (!raw.AETHER_URL) {
    ...
    if (envMode === 'cloud') {
      issues.push({ var: 'AETHER_URL', message: 'Required in cloud mode — sandbox routing, channels webhooks, and health checks will break', level: 'error' });
    } else {
      // Auto-derive for local mode so it "just works"
      const derived = `http://localhost:${port}/v1/router`;
      process.env.AETHER_URL = derived;
      if (result.success) (result.data as any).AETHER_URL = derived;
      console.warn(`[config] AETHER_URL not set — auto-derived: ${derived}`);
      issues.push({ var: 'AETHER_URL', message: `Not set — auto-derived to ${derived} (add to .env to silence this)`, level: 'warn' });
    }
  }
  ...
  if (warnings.length > 0) {
    ...
    console.warn('\x1b[33m  aether-api: Environment warnings\x1b[0m');
    console.warn('\x1b[33m' + '='.repeat(70) + '\x1b[0m');
  ...
  if (errors.length > 0) {
    ...
    console.error('\x1b[31m  aether-api: Environment validation FAILED — server cannot start\x1b[0m');
    console.error('\x1b[31m' + '='.repeat(70) + '\x1b[0m');
...
export const config = {
  ...
  INTERNAL_AETHER_ENV: env.INTERNAL_AETHER_ENV as InternalAetherEnv,
  // Billing is enabled when ENV_MODE is 'cloud' — no separate env var needed.
  AETHER_BILLING_INTERNAL_ENABLED: env.AETHER_BILLING_INTERNAL_ENABLED || env.ENV_MODE === 'cloud',
  AETHER_DEPLOYMENTS_ENABLED: env.AETHER_DEPLOYMENTS_ENABLED,
  ...
  DAYTONA_SNAPSHOT: env.DAYTONA_SNAPSHOT || `aether-sandbox-v${SANDBOX_VERSION}`,
  ...
  SANDBOX_IMAGE: env.SANDBOX_IMAGE || 'aether/computer:latest',
  AETHER_LOCAL_IMAGES: env.AETHER_LOCAL_IMAGES,
  ...
  /**
   * INTERNAL_SERVICE_KEY -- direction: aether-api -> sandbox.
   *
  ...
  AETHER_MASTER_URL: env.AETHER_MASTER_URL,
  OPENCODE_URL: env.OPENCODE_URL,
  AETHER_DATA_DIR: env.AETHER_DATA_DIR,
...
//   * Aether keys (user uses our keys):  1.2x provider cost (20% markup)
