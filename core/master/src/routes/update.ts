...
// The actual image pull + container recreate is handled by theapi
// (local-docker provider). This endpoint just signals readiness and version info.
updateRoutes.post(
  '/',
  describeRoute({
    tags: ['update'],
    description: 'Signal sandbox update. Actual image pull + recreate is handled byapi.',
    responses: {
  ...
  async (c) => {
    ...
    await setStatus({
      ...
      message: `Update to ${targetVersion} pending — container will be recreated byapi`,
      targetVersion,
    ...
    return c.json({
      success: true,
      message: `Update to ${targetVersion} acknowledged. Container recreate will be triggered byapi.`,
      currentVersion,
      targetVersion,
      action: 'recreate',
    });
