...
BUN_PTY_PKG="/ephemeral/acme-master/node_modules/bun-pty/package.json"
if [ -f "$BUN_PTY_PKG" ]; then
    ...
    echo "[fix-bun-pty]   Run: cd /ephemeral/acme-master && bun install"
fi
...
if [ -f "$MUSL_SO" ]; then
    for PTY_DIR in \
        /ephemeral/acme-master/node_modules/bun-pty/rust-pty/target/release \
...
if command -v bun >/dev/null 2>&1 && [ -d /ephemeral/acme-master/node_modules/bun-pty ]; then
    echo "[fix-bun-pty] Running bun-pty smoke test..."
    SMOKE_RESULT=$(cd /ephemeral/acme-master && bun -e "
      try {
