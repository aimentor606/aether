/**
 *  Projects — project CRUD + session-project linking + file gating.
 *
 * SQLite (aether.db) is the single source of truth.
...
export class ProjectManager {
	...
	async createProject(name: string, desc: string, customPath: string): Promise<ProjectRow> {
		...
		for (const d of [".aether/docs", ".aether/sessions"])
			await fs.mkdir(path.join(pp, d), { recursive: true })
		ensureGlobalMemoryFiles(import.meta.dir)
		await wm(path.join(pp, ".aether", "CONTEXT.md"), `# ${name}\n\n${desc || "No description."}\n`)
...
export function projectTools(mgr: ProjectManager, db: Database) {
	return {
		...
		project_list: tool({
			description: "List all projects from  SQLite.",
			args: {},
		...
		project_get: tool({
			...
			async execute(args: { name: string }): Promise<string> {
				...
				const contextPath = path.join(p.path, ".aether", "CONTEXT.md")
				return [
					`## ${p.name}`, ``, `**Path:** \`${p.path}\``,
...
export function projectStatusTransform(mgr: ProjectManager, getCurrentSessionId: () => string | null) {
	return async (_input: any, output: { messages: any[] }) => {
		// Only inject orchestrator reminder for the primary agent (), not sub-agents
		const isOrchestrator = (() => {
			for (const m of output.messages) {
				const agent = m?.info?.agent
				if (agent && agent !== "aether") return false
			}
			return true
		})()
		try {
			...
			for (let i = messages.length - 1; i >= 0; i--) {
				if (messages[i]?.info?.role === "user") {
					...
					messages[i].parts.push({ type: "text", text: `<aether_system type="project-status" source="aether-system">${statusXml}</aether_system>` })
					break
				}
			}
		} catch {}
