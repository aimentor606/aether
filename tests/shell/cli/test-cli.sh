#!/usr/bin/env bash
# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║  Test Suite: Embedded CLI (generated by get-acme.sh)                     ║
# ╚══════════════════════════════════════════════════════════════════════════════╝
...
# Extract the embedded CLI from get-acme.sh (between CLIPATH markers)
CLI_CONTENT=$(sed -n "/cat > .*\/acme.*<< 'CLIPATH'/,/^CLIPATH/p" "$SCRIPT" | sed '1d;$d')

if [ -z "$CLI_CONTENT" ]; then
  fail "could not extract embedded CLI from get-acme.sh"
  echo ""; echo "  0/$TOTAL passed"; echo ""
  exit 1
fi

pass "embedded CLI extracted from get-acme.sh"
