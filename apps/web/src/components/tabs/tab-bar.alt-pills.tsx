...
function TabContextMenu({ tab, position, onAction, onClose }: ContextMenuProps) {
  ...
  return (
    <div
      ref={menuRef}
      className="fixed z-[100] min-w-[190px] rounded-xl border border-border/50 bg-popover/98 backdrop-blur-sm shadow-xl shadow-black/10 p-1 animate-in fade-in-0 zoom-in-95 slide-in-from-top-1 duration-100"
...
function TabListDropdown({ tabs, activeTabId, onActivate, open, onOpenChange, getStatus }: TabListDropdownProps) {
  ...
  const renderTabItem = (tab: Tab) => {
    ...
    return (
      <DropdownMenuItem
        key={tab.id}
        className={cn(
          'flex items-center gap-2 text-[13px] rounded-lg py-1.5 px-2.5 cursor-pointer transition-colors duration-150',
          isActive
      ...
      >
        {tab.type === 'session' && (isBusy || pendingCount > 0) ? (
          <div className="relative flex-shrink-0 w-3.5 h-3.5 flex items-center justify-center">
            {isBusy && <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse" />}
  ...
  return (
    <DropdownMenu open={open} onOpenChange={(v) => { if (!v) setSearchQuery(''); onOpenChange(v); }}>
      ...
      <DropdownMenuContent align="end" className="min-w-[260px] max-w-[340px] max-h-[460px] flex flex-col p-0 rounded-xl">
        {tabs.length > 3 && (
        ...
        <div className="overflow-y-auto p-1 flex-1 space-y-0.5">
          ...
          {sessionTabs.length > 0 && otherTabs.length > 0 && <DropdownMenuSeparator className="my-1" />}
          {otherTabs.length > 0 && (
            <DropdownMenuGroup>
              {sessionTabs.length > 0 && (
                <DropdownMenuLabel className="text-[10px] font-medium text-muted-foreground/50 uppercase tracking-wider py-1 px-2.5">
...
function TabItem({
  ...
  return (
    <div
      ...
      className={cn(
        'group relative flex items-center h-7 text-[13px] leading-none select-none cursor-pointer',
        'transition-colors duration-150 rounded-md',
        isDashboard
          ? 'w-7 justify-center'
          : 'gap-1.5 px-2 md:px-2.5 max-w-[200px] min-w-[48px] md:min-w-[80px]',
        isActive
    ...
    >
      ...
      {isDragOver && dragSide === 'left' && (
        <div className="absolute left-0 top-0.5 bottom-0.5 w-[2px] bg-primary rounded-full z-10 animate-in fade-in-0 duration-150" />
      )}
      {isDragOver && dragSide === 'right' && (
        <div className="absolute right-0 top-0.5 bottom-0.5 w-[2px] bg-primary rounded-full z-10 animate-in fade-in-0 duration-150" />
      )}
      ...
      {tab.type === 'session' && (isBusy || pendingCount > 0) ? (
        ...
        <span className="flex-shrink-0 flex items-center justify-center w-4 h-4">
          {getFileIcon(tab.title || 'file', { className: 'h-3.5 w-3.5' })}
        </span>
      ) : (
        <span className="flex-shrink-0 flex items-center justify-center w-4 h-4">
      ...
      {!tab.pinned && !isDashboard && (
        <button
          ...
          className={cn(
            'flex-shrink-0 p-0.5 rounded transition-colors duration-100 cursor-pointer',
            'hover:bg-foreground/10',
            'hidden md:block',
            isActive
              ? 'md:opacity-50 md:hover:opacity-80'
              : 'md:opacity-0 md:group-hover:opacity-50 md:group-hover:hover:opacity-80',
          )}
...
export function TabBar() {
  ...
  if (orderedTabs.length === 0) {
    return <div className="flex-shrink-0 bg-sidebar h-[42px] md:h-[38px]" />;
  }
  ...
  return (
    <>
      <div
        ref={tabBarRef}
        className="flex-shrink-0 flex items-center bg-sidebar h-[42px] md:h-[38px] relative overflow-hidden"
        role="tablist"
      >
        ...
        <div
          ref={scrollRef}
          onWheel={handleWheel}
          className="flex-1 flex items-center gap-0.5 overflow-x-auto px-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']"
        >
          {orderedTabs.map((tab, index) => {
            ...
            return (
              <div key={tab.id} data-tab-id={tab.id} className="flex items-center relative flex-shrink-0">
                <TabItem
                  tab={tab}
                  index={index}
        ...
        <div className="flex-shrink-0 flex items-center gap-0.5 pr-2 relative z-20 bg-sidebar pl-1">
          {/* Fade edge — hidden when scrolled fully right */}
          <div ref={scrollFadeRef} className="absolute right-full top-0 bottom-0 w-4 bg-gradient-to-r from-transparent to-sidebar pointer-events-none transition-opacity duration-150" />
          {/* New tab button */}
          <Tooltip>
            <TooltipTrigger asChild>
              <button
