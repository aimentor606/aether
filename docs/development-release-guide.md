...
gh variable set AUTO_DEPLOY_DEV --repo aimentor606/aether --body true

# Disable
gh variable set AUTO_DEPLOY_DEV --repo aimentor606/aether --body false

# Check current value
gh variable list --repo aimentor606/aether
```

### Trigger a dev deploy manually

```bash
gh workflow run deploy-dev.yml --repo aimentor606/aether
```
...
```bash
gh workflow run snapshot-build.yml --repo aimentor606/aether -f version=dev-latest -f target=dev
```
...
gh workflow run snapshot-build.yml --repo aimentor606/aether -f version=0.8.30 -f target=prod
...
gh workflow run release.yml --repo aimentor606/aether \
  -f version="0.8.30" \
