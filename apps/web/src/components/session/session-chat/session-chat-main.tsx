...
export function SessionChat({
	...
	const addOptimisticUserMessage = useCallback(
		(messageId: string, text: string, partIds?: string[]) => {
			...
			useSyncStore
				.getState()
				.optimisticAdd(sessionId, info, parts as any);
		},
		[sessionId],
	);
	...
	useEffect(() => {
		...
		const attemptSend = (attempt: number) => {
			...
			sessionStorage.removeItem(
				`opencode_pending_send_failed:${sessionId}`,
			);
			...
			try {
				...
				if (raw) {
					...
					if (pendingOptions?.model) {
						const parsedPendingModel = parseModelKey(
							pendingOptions.model,
						);
					...
					if (pendingOptions?.variant) {
						...
						local.model.variant.set(
							pendingOptions.variant as string,
						);
					}
				}
			} catch {
				...
				client.session
					...
					.then((res) => {
						if (res.data) {
							useSyncStore
								.getState()
								.hydrate(sessionId, res.data as any);
							useSyncStore
								.getState()
					...
					.catch((error) => {
						logger.warn(
							"Failed to hydrate messages after prompt error",
							{ sessionId, error },
						);
						removeOptimisticUserMessage(messageID);
					});
			...
			const sendPendingPrompt = async () => {
				...
				const localFiles = pendingFiles.filter(
					(f): f is Extract<typeof f, { kind: "local" }> =>
						f.kind === "local",
				);
				...
				if (localFiles.length > 0) {
					...
					const uploadResults = await Promise.all(
						localFiles.map(async (af, index) => {
							const safeName = af.file.name.replace(
								/[^a-zA-Z0-9._-]/g,
								"_",
							);
							const uniqueName = `${uploadBatchTs}-${index}-${safeName}`;
							const uploadBlob = new File(
								[af.file],
								uniqueName,
								{ type: af.file.type },
							);
							...
							if (!results || results.length === 0) {
								throw new Error(
									`Failed to upload file: ${af.file.name}`,
								);
							}
							return {
								path: results[0].path,
	...
	useEffect(() => {
		...
		if (!cached || !cached.messageID || !cached.partID || !cached.text)
			return;
		...
		const hasMsg = currentMsgs.some(
			(m) => m.info.id === cached!.messageID,
		);
		const hasAnyUser = currentMsgs.some(
			(m) => m.info.role === "user",
		);
		...
		if (!hasMsg) {
			...
			if (parentID) {
				const parentExists = currentMsgs.some(
					(m) => m.info.id === parentID,
				);
	...
	useEffect(() => {
		...
		const timer = setTimeout(() => {
			const currentMsgs = useSyncStore
				.getState()
			...
			const currentType =
				syncStoreStatus?.type ?? legacyStoreStatus?.type;
	...
	useEffect(() => {
		...
		client.session
			...
			.catch((error) => {
				logger.warn(
					"Failed to recover messages after busy→idle transition",
					{ sessionId, error },
				);
			});
	...
	const pendingPermissions = useMemo(
		() =>
			Object.values(allPermissions).filter(
				(p) => p.sessionID === sessionId,
			),
		[allPermissions, sessionId],
	);
	const suppressedQuestionIdsRef = useRef<Map<string, number>>(new Map());
	const suppressQuestionFor = useCallback(
		(requestId: string, ms = 15000) => {
	...
	const [renderedQuestion, setRenderedQuestion] =
		useState<import("@/ui").QuestionRequest | null>(null);
				...
				if (parts) {
					const match = parts.find(
						(p) =>
							...
							(p as ToolPart).callID ===
								questionReq.tool!.callID,
					);
					if (match) {
						optimisticAnswersCache.set(match.id, {
							answers,
							input:
								(match as ToolPart).state?.input as Record<
									string,
									unknown
								> ?? {},
						});
	...
	useEffect(() => {
		...
		const offset =
			targetRect.top - scrollRect.top + scrollEl.scrollTop - 24;
		scrollEl.scrollTo({
	...
	const handleSend = useCallback(
		async (
			...
			while (
				(rawOptimisticMatch = rawOptimisticRegex.exec(text)) !== null
			) {
				const rawId = rawOptimisticMatch[1];
				if (
					sessionMentionsForOptimistic.some(
						(m) => m.value === rawId,
					)
				)
					continue;
			...
			if (allOptimisticSessionMentions.length > 0) {
				const refs = allOptimisticSessionMentions
					.map(
						(m) =>
							`<session_ref id="${m.value}" title="${m.label}" />`,
					)
					.join("\n");
			...
			if (uploadPlans.length > 0) {
				const uploadResults = await Promise.all(
					uploadPlans.map(async (plan) => {
						...
						if (!results || results.length === 0) {
							throw new Error(
								`Failed to upload file: ${plan.filename}`,
							);
						}
						return {
							path: results[0].path,
			...
			while (
				(rawMatch = rawSessionIdRegex.exec(textPrompt.text)) !== null
			) {
				const rawId = rawMatch[1];
				if (
					trackedSessionMentions.some((m) => m.value === rawId)
				)
					continue;
				const found = allSessions?.find((s: any) => s.id === rawId);
				if (found) {
			...
			const handleSendError = () => {
				...
				client.session
					...
					.catch((error) => {
						logger.warn(
							"Failed to hydrate messages after send error",
							{ sessionId, error },
						);
						removeOptimisticUserMessage(messageID);
					});
	...
	useEffect(() => {
		const onKeyDown = (e: KeyboardEvent) => {
			...
			if (withinWindow) {
				const currentCount = escDeadlineRef.current
					? Math.max(1, escCount)
					: 0;
				if (currentCount >= 2) {
					clearEscHint();
		...
		<div className="relative flex flex-col h-full bg-background">
			...
			{!readOnly && (
				<SessionChatInput
					...
					onVariantChange={(v) =>
						local.model.variant.set(v ?? undefined)
					}
					messages={messages}
					sessionId={sessionId}
					...
					questionButtonLabel={
						renderedQuestion ? questionAction.label : null
					}
