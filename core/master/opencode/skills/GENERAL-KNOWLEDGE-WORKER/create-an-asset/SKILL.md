...
"Snowflake returns the data.master compares actuals vs. obligations:
...
**Design foundation:** Follow the design-foundations principle of **1 accent + neutrals** (see `skills/GENERAL-KNOWLEDGE-WORKER/design-foundations/SKILL.md`). The prospect's brand primary becomes the accent; everything else should be neutral. If no brand colors are available, fall back to themaster neutral palette defaults: black/white or off-white neutrals plus one accent. Reserve color for emphasis — the viewer's eye goes where color is.
...
- Components:master, Workato DataGenie, Snowflake, PDF contracts

**Output structure:**
```
[Interactive canvas with 5 nodes]
Human →master → PDF Contracts → Workato → Snowflake
         ↓
...
| ai | 🤖 or brain SVG |master, AI Agent |
...
| Energy | `#16a34a` (green) |
| Default |master teal `#22808D` (see `skills/GENERAL-KNOWLEDGE-WORKER/design-foundations/SKILL.md`) |
