    ...
    const getInitialSession = async () => {
      try {
        ...
        if (currentSession?.user?.id) {
          const prevUserId = localStorage.getItem('acme-last-user-id');
          ...
          localStorage.setItem('acme-last-user-id', currentSession.user.id);
        }
      } catch (error) {
      } finally {
        setIsLoading(false);
      ...
      async (event, newSession) => {
        ...
        switch (event) {
          case 'SIGNED_IN': {
            ...
            if (newUserId) {
              localStorage.setItem('acme-last-user-id', newUserId);
            }
            break;
          }
          case 'SIGNED_OUT':
            ...
            setCachedAuthToken(null);
            clearUserLocalStorage();
            localStorage.removeItem('acme-last-user-id');
            break;
