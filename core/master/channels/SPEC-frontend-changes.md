...
- **Data source**: Postgres `channel_configs` table via `/v1/channels` API onapi
...
Actually even simpler — the frontend just needs to read from the sandbox's master API. We add a thin route onmaster:
