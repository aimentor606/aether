 ...
 */
const DesktopFileViewer = memo(function DesktopFileViewer({
  filePath,
}: {
  filePath: string;
}) {
 ...
 */
const DesktopFolderBrowser = memo(function DesktopFolderBrowser({
  folderPath,
}: {
  folderPath?: string;
}) {
...
const FolderWindow = memo(function FolderWindow({
  ...
  const folderName =
    (window.filePath || '/workspace').split('/').pop() || 'Files';
...
export const SandboxDesktop = memo(function SandboxDesktop({
  ...
  const spotlightFiles = workspaceFiles.map((file) => ({
    ...
    type:
      file.type === 'directory' ? ('directory' as const) : ('file' as const),
  ...
  const openToolWindow = useCallback(
    (toolIndex: number) => {
  ...
  const openFileInfoWindow = useCallback(
    (fileInfo: FileInfo) => {
  ...
  useEffect(() => {
    ...
    if (currentView === 'files') {
      ...
      const existingBrowserWindow = openWindows.find(
        (w) => w.type === 'browser',
      );
  ...
  const handleSystemAppClick = useCallback(
    (type: 'files' | 'browser' | 'terminal' | 'info' | 'spreadsheet') => {
      if (type === 'info') {
  ...
  const isFilesWindowOpen = openWindows.some(
    (w) => w.id === 'system-files' && !w.isMinimized,
  );
  const isBrowserWindowOpen = openWindows.some(
    (w) => w.id === 'system-browser' && !w.isMinimized,
  );
  const isTerminalWindowOpen = openWindows.some(
    (w) => w.id === 'system-terminal' && !w.isMinimized,
  );
  const isInfoWindowOpen = openWindows.some(
    (w) => w.id === 'system-info' && !w.isMinimized,
  );
  const isSpreadsheetWindowOpen = openWindows.some(
    (w) => w.id === 'system-spreadsheet' && !w.isMinimized,
  );
  ...
  const handleDesktopFileOpen = useCallback(
    (path: string, isDirectory: boolean) => {
  ...
  const handleDesktopFileEdit = useCallback(
    (path: string) => {
        ...
        <AnimatePresence>
          {visibleWindows.map((window) => {
            if (window.type === 'tool' && window.toolIndex !== undefined) {
              ...
              const isSuccess = isToolStreaming
                ? true
                : getActualSuccess(toolCall);
              ...
              return (
                <AppWindow
                  ...
                  icon={
                    ...
                    >
                      <ToolIcon
                        className={cn('w-2.5 h-2.5', colorScheme.iconColor)}
                      />
                    </div>
                  }
              ...
              return (
                <AppWindow
                  ...
                  icon={
                    <div className="w-4 h-4 flex items-center justify-center">
                      {getFileIconByName(
                        window.fileInfo.name,
                        window.fileInfo.isDirectory,
                      )}
                    </div>
                  }
                  isActive={activeWindowId === window.id}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="relative flex-1 overflow-hidden">{renderDesktop()}</div>
        ...
        <QuickLaunch
          ...
          onFileSelect={(path) => {
            const isDir =
              spotlightFiles.find((f) => f.path === path)?.type === 'directory';
