...
export const useSyncStore = create<SyncState>()((set, get) => ({
  ...
  removeMessage: (sessionID, messageID) =>
    set((s) => {
      ...
      const idx =
        result.found && list[result.index]?.id === messageID
          ? result.index
    ...
    set((s) => {
      ...
      for (const m of msgs) {
        ...
        const reconciled = inParts.map((inP) => {
          ...
          const isTextLike =
            (inP as any)?.type === 'text' || (inP as any)?.type === 'reasoning';
          if (
            isTextLike &&
    ...
    switch (event.type) {
      case 'message.updated': {
        ...
        if ((info as any).error)
          console.log('[sync-store] message.updated with error', {
            id: info.id,
      ...
      case 'message.part.updated': {
        ...
        const eventSessionID = (event.properties as { sessionID?: string })
          ?.sessionID;
        let resolvedSessionID = (part as any).sessionID ?? eventSessionID;
        ...
        const bsResult =
          existingMsgs &&
          Binary.search(existingMsgs, part.messageID, (m) => m.id);
        const exists =
          existingMsgs &&
          ((bsResult &&
            bsResult.found &&
            existingMsgs[bsResult.index]?.id === part.messageID) ||
      ...
      case 'message.part.delta': {
        ...
        const partExists =
          partList && partList.some((p) => p.id === props.partID);
        if (!partExists) {
      ...
      case 'session.error': {
        ...
        console.log('[sync-store] session.error received', {
          sid,
          error: props.error,
        });
        ...
        set((s) => {
          ...
          console.log(
            '[sync-store] session.error — current messages for session:',
            msgs.length,
            msgs.map((m) => ({
              id: m.id,
              role: m.role,
