...
export async function pullImage(endpoint: ResolvedEndpoint, image: string): Promise<StepResult> {
  ...
  await execOnHost(
    endpoint,
    'for u in $(systemctl list-units --all --no-legend "aether-pull-*" | awk "{print \\$1}"); do systemctl stop "$u" 2>/dev/null; systemctl reset-failed "$u" 2>/dev/null; done; true',
    10,
  );
  ...
  const unitName = `aether-pull-${Date.now()}`;
...
export async function stopAndStartContainer(
  ...
  const unitName = `aether-update-${Date.now()}`;

  await execOnHost(
    endpoint,
    `echo '${b64}' | base64 -d > /tmp/aether-update.sh && chmod +x /tmp/aether-update.sh`,
    5,
  );

  const result = await execOnHost(
    endpoint,
    `systemctl reset-failed ${unitName} 2>/dev/null || true; systemd-run --unit=${unitName} --description="Aether sandbox update" /tmp/aether-update.sh`,
    15,
  );
