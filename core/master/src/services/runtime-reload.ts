/**
 * Runtime Reload — Nuclear restart for the entire stack.
 *
 * Two modes:
 ...
 *   svc-master      — This process (port 8000)
 */
...
export async function initiateRuntimeReload(mode: ReloadMode): Promise<ReloadResult> {
  ...
  result.steps.push('Restartingmaster last — s6 will respawn with fresh state')
  result.success = true

  // Self-restart — deferred so HTTP response goes out first
  setTimeout(() => {
    console.log('[runtime-reload] Full restart: killing master — s6 will respawn')
    try {
      Bun.spawn(['s6-svc', '-r', '/run/service/svc-master'], {
