...
export function RateLimitersPanel({ limiters }: RateLimitersPanelProps) {
  ...
  const getUsagePercent = (stats: RateLimiterStats): number => {
    ...
    if (
      ...
      stats.max_requests &&
      stats.requests_in_window !== undefined
    ) {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Request throttling and traffic control
        </CardDescription>
      </CardHeader>
      <CardContent>
                ...
                <div key={name} className="space-y-2">
                  ...
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    {stats.type === 'token_bucket' && (
                      <>
                        <span>
                          Tokens: {stats.tokens?.toFixed(0)} / {stats.capacity}
                        </span>
                    ...
                    {stats.type === 'sliding_window' && (
                      <>
                        <span>
                          Requests: {stats.requests_in_window} /{' '}
                          {stats.max_requests}
                        </span>
                    ...
                    {stats.type === 'adaptive' && (
                      <>
                        ...
                        <span>
                          Range: {stats.min_rate} - {stats.max_rate}
                        </span>
                      </>
                    )}
                  </div>
