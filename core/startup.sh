...
#   - Set up git, tool deps, env guards (98-aether-env.sh)
...
echo "[startup] Preparing aether sandbox..."
...
# Old images created /workspace/.opencode as a symlink to /workspace/.aether/.opencode.
...
# Old images created /workspace/.secrets as a symlink to /workspace/.aether/secrets.
...
# For fresh workspaces, aether-env-setup.sh writes info/exclude AFTER git init
...
if command -v ocx >/dev/null 2>&1; then
  ...
  su -s /bin/sh abc -c 'ocx registry add https://master.aether-registry.pages.dev --name aether --cwd /workspace -q' 2>/dev/null || true
...
if [ ! -e /ephemeral/aether-master ]; then
  echo "[startup] WARNING: /ephemeral/aether-master not found! Rebuild the Docker image."
fi
...
if [ -x /ephemeral/aether-master/scripts/install-channel-clis.sh ]; then
