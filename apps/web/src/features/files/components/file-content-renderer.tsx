...
const PdfRenderer = lazy(() =>
  import('@/components/file-renderers/pdf-renderer').then((m) => ({
    default: m.PdfRenderer,
  })),
);
const DocxRenderer = lazy(() =>
  import('@/components/file-renderers/docx-renderer').then((m) => ({
    default: m.DocxRenderer,
  })),
);
const VideoRenderer = lazy(() =>
  import('@/components/file-renderers/video-renderer').then((m) => ({
    default: m.VideoRenderer,
  })),
);
const CsvRenderer = lazy(() =>
  import('@/components/file-renderers/csv-renderer').then((m) => ({
    default: m.CsvRenderer,
  })),
);
const XlsxRenderer = lazy(() =>
  import('@/components/file-renderers/xlsx-renderer').then((m) => ({
    default: m.XlsxRenderer,
  })),
);
const PptxRenderer = lazy(() =>
  import('@/components/file-renderers/pptx-renderer').then((m) => ({
    default: m.PptxRenderer,
  })),
);
const ImageRenderer = lazy(() =>
  import('@/components/file-renderers/image-renderer').then((m) => ({
    default: m.ImageRenderer,
  })),
);
...
export function FileContentRenderer({
  ...
  useEffect(() => {
    ...
    async function check() {
      try {
        const res = await fetch(htmlHealthUrl, {
          method: 'GET',
          credentials: 'include',
        });
        if (cancelled) return;
  ...
  const blobPath =
    isBlobCategory(fileCategory) || isHeicImage ? filePath : null;
  const {
    blobUrl,
    blob: rawBlob,
  ...
  return (
    <div className={cn('flex flex-col h-full', className)}>
      ...
      <div
        className={cn('flex-1', readOnly ? 'overflow-auto' : 'overflow-hidden')}
      >
        {/* Loading */}
        {showLoadingState && (
          <div className="flex items-center justify-center h-full">
        ...
        {!isLoading &&
          ...
          !imageDataUrl &&
          !isHeicImage &&
          !['pdf', 'docx', 'pptx', 'xlsx', 'sqlite', 'video', 'audio'].includes(
            fileCategory,
          ) && (
            <div className="flex flex-col items-center justify-center h-full gap-3 p-8 text-center">
              <div className="h-12 w-12 rounded-xl bg-muted/50 flex items-center justify-center">
...
function JsonTreeView({ content }: { content: string }) {
  ...
  if (parsed === null) {
    return (
      <div className="p-4 text-sm text-red-500/70 font-mono">Invalid JSON</div>
    );
  }
      ...
      <div style={{ paddingLeft: depth * 20 }} className="break-all">
        ...
        {isUrl && (
          <a
            ...
            className="ml-1 text-blue-400/60 hover:text-blue-400 text-xs"
          >
            open
          </a>
        )}
