...
export const useAcmeComputerStore = create<AcmeComputerState>()(
  devtools(
    (set, get) => ({
      ...initialState,
      ...
      setSandboxContext: (sandboxId: string | null) => {
        ...
        if (currentSandboxId !== sandboxId) {
          console.log('[AcmeComputerStore] Sandbox context changed:', currentSandboxId, '->', sandboxId);
      ...
      setIsSidePanelOpen: (open: boolean) => {
        ...
        const update: Partial<AcmeComputerState> = { isSidePanelOpen: open };
        if (sessionId) {
      ...
      openSidePanel: () => {
        const sessionId = get()._activeSessionId;
        const update: Partial<AcmeComputerState> = { isSidePanelOpen: true };
        if (sessionId) {
      ...
      closeSidePanel: () => {
        const sessionId = get()._activeSessionId;
        const update: Partial<AcmeComputerState> = { isSidePanelOpen: false, isExpanded: false };
        if (sessionId) {
      ...
      reset: () => {
        console.log('[AcmeComputerStore] Full reset');
        useFilesStore.getState().reset();
        set(initialState);
      },
    }),
    {
      name: 'acme-computer-store',
    }
  )
);
...
// Sandbox context
export const useAcmeComputerSandboxId = () =>
  useAcmeComputerStore((state) => state.currentSandboxId);

export const useSetSandboxContext = () =>
  useAcmeComputerStore((state) => state.setSandboxContext);
...
export const useIsSidePanelOpen = () =>
  useAcmeComputerStore((state) => state.isSidePanelOpen);

export const useSetIsSidePanelOpen = () =>
  useAcmeComputerStore((state) => state.setIsSidePanelOpen);

export const useIsExpanded = () =>
  useAcmeComputerStore((state) => state.isExpanded);

export const useToggleExpanded = () =>
  useAcmeComputerStore((state) => state.toggleExpanded);
