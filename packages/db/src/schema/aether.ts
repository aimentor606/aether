...
export const sandboxProviderEnum = aetherSchema.enum('sandbox_provider', [
  'daytona',
  'local_docker',
  'justavps',
]);

export const deploymentStatusEnum = aetherSchema.enum('deployment_status', [
  'pending',
  'building',
  'deploying',
  'active',
...
export const deploymentSourceEnum = aetherSchema.enum('deployment_source', [
  'git',
  'code',
  'files',
  'tar',
...
export const apiKeyStatusEnum = aetherSchema.enum('api_key_status', [
  'active',
  'revoked',
  'expired',
]);
...
export const accounts = aetherSchema.table(
  'accounts',
  {
...
export const accountMembers = aetherSchema.table(
  'account_members',
  {
...
export const sandboxes = aetherSchema.table(
  'sandboxes',
  {
...
export const poolResources = aetherSchema.table(
  'pool_resources',
  {
...
export const poolSandboxes = aetherSchema.table(
  'pool_sandboxes',
  {
    id: uuid('id').defaultRandom().primaryKey(),
...
export const deployments = aetherSchema.table(
  'deployments',
  {
...
export const aetherApiKeys = aetherSchema.table(
  'api_keys',
  {
...
export const integrationCredentials = aetherSchema.table(
...
export const integrations = aetherSchema.table(
  'integrations',
  {
...
export const sandboxIntegrations = aetherSchema.table(
...
export const serverEntries = aetherSchema.table(
  'server_entries',
  {
...
export const oauthClients = aetherSchema.table(
  'oauth_clients',
  {
...
export const oauthAuthorizationCodes = aetherSchema.table(
...
export const oauthAccessTokens = aetherSchema.table(
  'oauth_access_tokens',
  {
    id: uuid('id').defaultRandom().primaryKey(),
...
export const oauthRefreshTokens = aetherSchema.table(
...
export const aetherApiKeysRelations = relations(aetherApiKeys, ({ one }) => ({
  sandbox: one(sandboxes, {
    fields: [aetherApiKeys.sandboxId],
    references: [sandboxes.sandboxId],
  }),
}));
...
export const billingCustomers = aetherSchema.table(
  'billing_customers',
  {
    accountId: uuid('account_id').notNull(),
...
export const creditAccounts = aetherSchema.table(
  'credit_accounts',
  {
    ...
    createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
    lastGrantDate: timestamp('last_grant_date', { withTimezone: true }),
    tier: varchar('tier', { length: 50 }).default('free'),
    billingCycleAnchor: timestamp('billing_cycle_anchor', { withTimezone: true }),
    nextCreditGrant: timestamp('next_credit_grant', { withTimezone: true }),
    ...
    canCancelAfter: timestamp('can_cancel_after', { withTimezone: true }),
    lastRenewalPeriodStart: bigint('last_renewal_period_start', { mode: 'number' }),
    paymentStatus: text('payment_status').default('active'),
    lastPaymentFailure: timestamp('last_payment_failure', { withTimezone: true }),
    scheduledTierChange: text('scheduled_tier_change'),
    scheduledTierChangeDate: timestamp('scheduled_tier_change_date', { withTimezone: true }),
    ...
    revenuecatCancelledAt: timestamp('revenuecat_cancelled_at', { withTimezone: true }),
    revenuecatCancelAtPeriodEnd: timestamp('revenuecat_cancel_at_period_end', { withTimezone: true }),
    revenuecatPendingChangeProduct: text('revenuecat_pending_change_product'),
    revenuecatPendingChangeDate: timestamp('revenuecat_pending_change_date', { withTimezone: true }),
    ...
    autoTopupLastCharged: timestamp('auto_topup_last_charged', { withTimezone: true }),
  },
  (table) => [
...
export const creditLedger = aetherSchema.table(
  'credit_ledger',
  {
...
export const creditUsage = aetherSchema.table('credit_usage', {
  id: uuid().defaultRandom().primaryKey().notNull(),
  accountId: uuid('account_id').notNull(),
...
export const accountDeletionRequests = aetherSchema.table('account_deletion_requests', {
  id: uuid().defaultRandom().primaryKey().notNull(),
  accountId: uuid('account_id').notNull(),
  userId: uuid('user_id').notNull(),
...
export const creditPurchases = aetherSchema.table('credit_purchases', {
  id: uuid().defaultRandom().primaryKey().notNull(),
  accountId: uuid('account_id').notNull(),
  amountDollars: numeric('amount_dollars', { precision: 10, scale: 2 }).notNull(),
...
export const tunnelStatusEnum = aetherSchema.enum('tunnel_status', [
  'online',
  'offline',
  'connecting',
]);

export const tunnelCapabilityEnum = aetherSchema.enum('tunnel_capability', [
  'filesystem',
  'shell',
  'network',
  'apps',
...
export const tunnelPermissionStatusEnum = aetherSchema.enum('tunnel_permission_status', [
  'active',
  'revoked',
  'expired',
]);

export const tunnelPermissionRequestStatusEnum = aetherSchema.enum('tunnel_permission_request_status', [
  'pending',
  'approved',
  'denied',
  'expired',
...
export const tunnelConnections = aetherSchema.table(
  'tunnel_connections',
  {
...
export const tunnelPermissions = aetherSchema.table(
  'tunnel_permissions',
  {
...
export const tunnelPermissionRequests = aetherSchema.table(
...
export const tunnelAuditLogs = aetherSchema.table(
  'tunnel_audit_logs',
  {
...
export const tunnelDeviceAuthStatusEnum = aetherSchema.enum('tunnel_device_auth_status', [
  'pending',
  'approved',
  'denied',
  'expired',
...
export const tunnelDeviceAuthRequests = aetherSchema.table(
...
export const platformRoleEnum = aetherSchema.enum('platform_role', [
  'user',
  'admin',
  'super_admin',
]);

export const platformUserRoles = aetherSchema.table(
  'platform_user_roles',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    accountId: uuid('account_id').notNull(),
...
export const accessRequestStatusEnum = aetherSchema.enum('access_request_status', [
  'pending',
  'approved',
  'rejected',
]);

export const platformSettings = aetherSchema.table(
  'platform_settings',
  {
...
export const accessAllowlist = aetherSchema.table(
  'access_allowlist',
  {
    id: uuid('id').defaultRandom().primaryKey(),
...
export const accessRequests = aetherSchema.table(
  'access_requests',
  {
    id: uuid('id').defaultRandom().primaryKey(),
