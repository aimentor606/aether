 ...
 *
 * FINDING K-3: Sandbox keys (aether_sb_*) have the same access level
 * as user keys (aether_*) in all routes that use combinedAuth or apiKeyAuth.
 *
...
describe('Code Review: Sandbox Key Privilege Escalation', () => {
  ...
  describe('[MEDIUM] No key type enforcement in middleware', () => {
    test('apiKeyAuth accepts both aether_ and aether_sb_ tokens', () => {
  ...
  describe('Routes accessible with sandbox key', () => {
    ...
    for (const route of routesWithCombinedAuth) {
      test(`${route} uses combinedAuth — sandbox key would be accepted`, () => {
        // A sandbox agent (which has a aether_sb_ key) can access these routes
