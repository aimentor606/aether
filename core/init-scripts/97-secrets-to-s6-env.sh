...
# (opencode, acme-master, triggers) have their env vars.
#
# Recovery chain (in priority order):
#   1. Docker env vars (injected at container create) → always present on first boot
#   2. /workspace/.secrets/.bootstrap-env.json → core vars (ACME_TOKEN, etc.)
#   3. /workspace/.secrets/.secrets.json → all user secrets (API keys, etc.)
#
# The SecretStore encrypts with a DEDICATED encryption key (NOT ACME_TOKEN).
# This means secrets survive ACME_TOKEN changes, API restarts, and rotations.
# ─────────────────────────────────────────────────────────────────────────────
...
SEED_FILE="/ephemeral/acme-master/seed-env.json"
...
# Bootstrap file contains plaintext core vars (ACME_TOKEN, ACME_API_URL,
# INTERNAL_SERVICE_KEY). These are needed BEFORE the SecretStore can decrypt
# anything (ACME_TOKEN was the old encryption key; now it's just identity).
if [ -f "$BOOTSTRAP_FILE" ] && [ -d "$S6_ENV_DIR" ]; then
  echo "[Acme] Restoring core env vars from bootstrap file..."
  bun -e "
    ...
    if (n > 0) console.log('[Acme] Restored ' + n + ' core env var(s) from bootstrap');
  " || echo "[Acme] WARN: bootstrap restore failed (non-fatal)"
fi

# ── 3. Seed template keys (first run only) ──────────────────────────────────
if [ ! -f "$SECRETS_FILE" ] && [ -f "$SEED_FILE" ]; then
  echo "[Acme] First run — seeding template keys into SecretStore"
  bun -e "
    const { SecretStore } = require('/ephemeral/acme-master/src/services/secret-store.ts');
    ...
    console.log('[Acme] Seeded ' + n + ' template keys');
  " || echo "[Acme] WARN: template key seed failed"
fi
...
if [ -f "$SECRETS_FILE" ]; then
  echo "[Acme] Syncing secrets into s6 container environment..."
  bun "/ephemeral/acme-master/src/scripts/sync-s6-env.ts" || echo "[Acme] WARN: secret sync failed"
else
  echo "[Acme] No secrets file yet — skipping secret sync"
fi

# ── 5. Fix s6 env dir ownership ─────────────────────────────────────────────
# Sync ran as root. acme-master (abc) needs write access at runtime.
if [ -d "$S6_ENV_DIR" ]; then
  chown -R abc:users "$S6_ENV_DIR"
fi

echo "[Acme] Secrets recovery complete"
