...
export function useTelegramConnect() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      try {
        const res = await fetch(`${sandboxUrl}/aether/channels/setup/telegram`, {
          method: 'POST',
          headers,
...
export function useSlackGenerateManifest() {
  ...
  return useMutation({
    mutationFn: async ({ publicUrl, botName }: {
      ...
      const token = await getAuthToken();
      const res = await fetch(`${sandboxUrl}/aether/channels/slack-manifest`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
...
export function useSlackConnect() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      // Try sandbox setup endpoint first (matches web)
      try {
        const res = await fetch(`${sandboxUrl}/aether/channels/setup/slack`, {
          method: 'POST',
          headers,
          body: JSON.stringify({
            botToken,
