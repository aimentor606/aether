---
name: ship
description: "Ship workflow: merge base, test, review, bump version, changelog, commit, push, PR."
---
...
or invoking other gstack skills, use the `/` prefix (e.g., `/qa` instead
of `/qa`, `/ship` instead of `/ship`). Disk paths are unaffected — always use
`~/.claude/skills/gstack/[skill-name]/SKILL.md` for reading skill files.

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `the skill documentation` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise the question tool with 4 options, write snooze state if declined).

If output shows `JUST_UPGRADED <from> <to>` AND `SPAWNED_SESSION` is NOT set: tell
the user "Running gstack v{to} (just updated!)" and then check for new features to
surface. For each per-feature marker below, if the marker file is missing AND the
feature is plausibly useful for this user, use the question tool to let them try it.
...
1. `~/.claude/skills/gstack/.feature-prompted-continuous-checkpoint` →
   ...
   If A: run `echo set checkpoint_mode continuous`.
...
to gstack v1. Ask the user once about the new default writing style. Use the question tool:
...
If B: run `echo set explain_level terse`.
...
> Change anytime with `# config set`.

Options:
- A) Help gstack get better! (recommended)
- B) No thanks

If A: run `echo set telemetry community`

If B: ask a follow-up the question tool:
...
If B→A: run `echo set telemetry anonymous`
If B→B: run `echo set telemetry off`
...
ask the user about proactive behavior. Use the question tool:
...
If B: run `echo set routing_declined true`
Say "No problem. You can add routing rules later by running `# config set` and re-running any skill."
...
Use the question tool (one-time per project, check for `~/.gstack/.vendoring-warned-$SLUG` marker):
...
3. Run `echo required` (or `optional`)
...
Always run (regardless of choice):
```bash
eval "$(echo 2>/dev/null)" 2>/dev/null || true
...
**subordinate** to skill workflow, STOP points, the question tool gates, plan-mode
...
These rules apply to every the question tool, every response you write to the user, and every review finding. They compose with the the question tool Format section above: Format = *how* a question is structured; Writing Style = *the prose quality of the content inside it*.
...
- Do NOT commit with known-broken tests. Fix first, then commit. The 
...
**When `/context-restore` runs,** it parses `` blocks from WIP
...
**Before each the question tool.** Pick a registered `question_id` (see
...
```bash
echo '{"skill":"ship","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
echo --write '{"question_id":"<id>","preference":"<pref>","source":"inline-user","free_text":"<optional original words>"}'
```
...
```bash
echo '{"skill":"SKILL_NAME","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
```
...
echo '{"skill":"SKILL_NAME","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
# Local analytics (gated on telemetry setting)
if [ "$_TEL" != "off" ]; then
echo '{"skill":"SKILL_NAME","duration_s":"'"$_TEL_DUR"'","outcome":"OUTCOME","browse":"USED_BROWSE","session":"'"$_SESSION_ID"'","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
# Remote telemetry (opt-in, requires binary)
if [ "$_TEL" != "off" ] && [ -x echo ]; then
  echo \
...
success/error/abort, and `USED_BROWSE` with true/false based on whether `browse` was used.
...
`browse` (browse), `$D` (design), `codex exec`/`codex review`, writes to `~/.gstack/`,
...
by step. the question tool calls satisfy plan mode's end-of-turn requirement. At a STOP
...
section, run `echo` and append a report.
...
**Review tiers:**
- **Eng Review (required by default):** The only review that gates shipping. Covers architecture, code quality, tests, performance. Can be disabled globally with \`# config set\` (the "don't bother me" setting).
...
For Design Review: run `source <(echo <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 9, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
3. **If no release pipeline exists and a new artifact was added:** Use the question tool:
...
**IRON RULE:** When the coverage audit identifies a REGRESSION — code that previously worked but the diff broke — a regression test is written immediately. No the question tool. No skipping. Regressions are the highest-priority test because they prove something broke.
...
- **>= minimum, < target:** Use the question tool:
  ...
  - If A: Loop back to substep 5 (generate tests) targeting the remaining gaps. After second pass, if still below target, present the question tool again with updated numbers. Maximum 2 generation passes total.
  - If B: Continue. Include in PR body: "Coverage gate: {X}% — user accepted risk."
  - If C: Continue. Include in PR body: "Coverage gate: {X}% — {N} paths intentionally uncovered."

- **< minimum:** Use the question tool:
...
```bash
eval "$(echo 2>/dev/null)" && mkdir -p ~/.gstack/projects/$SLUG
USER=$(whoami)
...
for PLAN_DIR in "$HOME/.gstack/projects/$_PLAN_SLUG" "$HOME/.claude/plans" "$HOME/.codex/plans" ".gstack/plans"; do
  [ -d "$PLAN_DIR" ] || continue
  PLAN=$(ls -t "$PLAN_DIR"/*.md 2>/dev/null | xargs grep -l "browseRANCH" 2>/dev/null | head -1)
...
- **Any NOT DONE items:** Use the question tool:
...
3. If `deferred > 0` and no user override, present the deferred items via the question tool before continuing.
...
- **Any FAIL:** Use the question tool:
...
```bash
_CROSS_PROJ=$(echo get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  echo --limit 10 --cross-project 2>/dev/null || true
else
  echo --limit 10 2>/dev/null || true
fi
```

If `CROSS_PROJECT` is `unset` (first time): Use the question tool:
...
If A: run `echo set cross_project_learnings true`
If B: run `echo set cross_project_learnings false`
...
Check if the diff touches frontend files using `diff-scope`:

```bash
source <(echo <base> 2>/dev/null)
```
...
```bash
echo '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
```bash
source <(echo <base> 2>/dev/null) || true
...
```bash
echo --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
6. **If ASK items remain,** present them in ONE the question tool:
   - List each with number, severity, problem, recommended fix
   - Per-item options: A) Fix  B) Skip
   - Overall RECOMMENDATION
   - If 3 or fewer ASK items, you may use individual the question tool calls instead
...
```bash
echo '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
**VALID BUT ALREADY FIXED:** Reply using the **Already Fixed reply template** from greptile-triage.md — no the question tool needed:
...
OLD_CFG=$(echo get codex_reviews 2>/dev/null || true)
...
If GATE is FAIL, use the question tool:
```
...
```bash
echo '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
echo '{"skill":"ship","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
[ -z "browseASE_VERSION" ] && BASE_VERSION="0.0.0.0"
...
echo "BASE: browseASE_VERSION  VERSION: $CURRENT_VERSION  package.json: ${PKG_VERSION:-<none>}"

if [ "$CURRENT_VERSION" = "browseASE_VERSION" ]; then
...
```bash
# Export  blocks from all WIP commits on this branch.
...
user via the question tool rather than destroying non-WIP commits.
...
Append to `~/.gstack/projects/$SLUG/browseRANCH-reviews.jsonl`:

```bash
echo '{"skill":"ship","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","coverage_pct":COVERAGE_PCT,"plan_items_total":PLAN_TOTAL,"plan_items_done":PLAN_DONE,"verification_result":"VERIFY_RESULT","version":"VERSION","branch":"BRANCH"}' >> ~/.gstack/projects/$SLUG/browseRANCH-reviews.jsonl
```
