/**
 * LSS (Local Semantic Search) API — semantic search via the Acme Master endpoint.
 *
 * Calls the `/lss/search` route on Acme Master, which spawns the `lss` CLI
...
export async function searchLss(
  ...
  // Build search URL — the server URL points to Acme Master (port 8000),
