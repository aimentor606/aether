/**
 * OpenAPI Spec Merger — combines master and OpenCode specs into one.
 *
 * Strategy:
 * - master spec is generated via hono-openapi generateSpecs()
 * - OpenCode spec is fetched live from localhost:{OPENCODE_PORT}/doc
 * - Paths are merged (master wins conflicts like /file, /file/content)
...
// Sandbox Gateway tags (from master describeRoute)
const GATEWAY_TAGS: Array<{ name: string; description: string }> = [
  ...
  { name: 'GW: Secrets',      description: 'Environment variable / secret management (encrypted at rest via AETHER_TOKEN)' },
...
// Map from old master tag names to new GW: prefixed names
 ...
 Spec: OpenAPISpec,
  ...
  const merged: OpenAPISpec = {
    ...
    info: {
      ...
      version: '1.0.0',
      description:
        'Unified API reference for the sandbox.\n\n' +
        '**Sandbox Gateway** — the sandbox infrastructure layer (master). ' +
        ...
        'Each sandbox gets an auto-generated token (`aether_sb_` prefix, 42 chars) injected as the `AETHER_TOKEN` environment variable. ' +
    ...
    servers: [{ url: 'https://YOUR_AETHER_API/v1/p/{sandboxId}/8000', description: 'Replace with your API base URL' }],
    tags: [],
    paths: {},
    components: {
      schemas: {},
      securitySchemes: {
       ApiKey: {
          ...
          description:
            ...
            'Pass as: `Authorization: Bearer<your-key>`',
        },
        SupabaseJWT: {
          type: 'http',
          scheme: 'bearer',
  ...
  // 1) Add all master paths first (they take priority)
  ...
  // 2) Add OpenCode paths (skip conflicts where master already has them)
  if (openCodeSpec) {
    for (const [path, methods] of Object.entries(openCodeSpec.paths || {})) {
      if (merged.paths[path]) {
        // Conflict — master wins, but merge any missing methods
        for (const [method, details] of Object.entries(methods as Record<string, any>)) {
    ...
    const kmSchemas =Spec.components?.schemas || {}
