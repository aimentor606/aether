...
/**
 * Static Acme Box 3D display — no scroll, LED always on, gentle idle rotation.
...
function AcmeBoxModel({ isDark }: { isDark: boolean }) {
  const groupRef = useRef<THREE.Group>(null!);
  const innerRef = useRef<THREE.Group>(null!);
  const { scene } = useGLTF('/models/acme_box_2.glb');
  ...
  useEffect(() => {
    ...
    scene.traverse((child) => {
      if (child instanceof THREE.Mesh && child.material) {
        ...
        const isLogo = child.name === 'acme_logo'
          || child.parent?.name === 'acme_logo'
          || child.name === 'TEXTS'
...
export default function AcmeBoxDisplay({ className }: { className?: string }) {
...
useGLTF.preload('/models/acme_box_2.glb');
