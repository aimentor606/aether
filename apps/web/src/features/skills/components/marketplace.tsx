...
async function installComponentWithOcx(componentName: string) {
  ...
  const isInstalled =
    normalized.includes('installed') || normalized.includes('done');
  if (
    !isInstalled &&
    (normalized.includes('not found') ||
      normalized.includes('failed to') ||
      normalized.includes('registry alias'))
  ) {
    throw new Error(
      output.replace(/[\x00-\x1F\x7F-\x9F]/g, '').trim() || 'Install failed',
    );
  }
  try {
    const client = getClient();
...
function ComponentDetailModal({
  ...
  const { data, isLoading, error } = useMarketplaceComponent(
    open ? (component?.name ?? null) : null,
  );
  ...
  const isInstalled = component
    ? installedSkills.has(component.name.toLowerCase())
    : false;
  const isInstalling = component ? installing.includes(component.name) : false;

  const activePath = useMemo(() => {
    if (!bundle) return null;
    if (selectedPath && bundle.files.some((f) => f.path === selectedPath))
      return selectedPath;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl w-[calc(100vw-2rem)] h-[80vh] grid grid-rows-[auto_1fr] gap-0 p-0 border-border/50 bg-background overflow-hidden">
        ...
        <div className="px-6 sm:px-8 py-5 border-b border-border/50">
          <div className="flex items-start justify-between gap-6 pr-8">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <Button
                ...
                onClick={() =>
                  component && navigator.clipboard.writeText(component.name)
                }
              >
                Copy
              </Button>
          ...
          {installError && (
            <p className="mt-3 text-xs text-destructive">{installError}</p>
          )}
        </div>
...
export function Marketplace() {
  ...
  const counts = useMemo(() => {
    ...
    return {
      all: list.length,
      installed: list.filter((i) => installedSkills.has(i.name.toLowerCase()))
        .length,
  ...
  const filtered = useMemo(() => {
    ...
    return (components ?? []).filter((c) => {
      ...
      switch (activeFilter) {
        case 'installed':
          return installedSkills.has(c.name.toLowerCase());
        case 'skills':
          return c.type === 'ocx:skill';
        case 'agents':
          return c.type === 'ocx:agent';
        case 'tools':
          return c.type === 'ocx:tool';
        case 'plugins':
          return c.type === 'ocx:plugin';
        default:
          return true;
      }
    });
