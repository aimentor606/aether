    ...
    const getInitialSession = async () => {
      try {
        ...
        if (currentSession?.user?.id) {
          const prevUserId = localStorage.getItem('aether-last-user-id');
          ...
          localStorage.setItem('aether-last-user-id', currentSession.user.id);
        }
      } catch (error) {
      } finally {
        setIsLoading(false);
      ...
      async (event, newSession) => {
        ...
        switch (event) {
          case 'SIGNED_IN': {
            ...
            if (newUserId) {
              localStorage.setItem('aether-last-user-id', newUserId);
            }
            break;
          }
          case 'SIGNED_OUT':
            ...
            setCachedAuthToken(null);
            clearUserLocalStorage();
            localStorage.removeItem('aether-last-user-id');
            break;
