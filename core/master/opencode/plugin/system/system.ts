/**
 *  System Plugin — THE single plugin for the entire  environment.
 *
 ...
 *  provides its own: agent_spawn/message/stop/status + task_create/list/update/done/delete.
 *
 * opencode.jsonc: "./plugin/aether-system/aether-system.ts"
 */
...
import { resolveWorkspaceRoot, ensureDir } from "./lib/paths"

const SystemPlugin: Plugin = async (ctx) => {
	const { client } = ctx

	// ── Core infra ──
	const workspaceRoot = resolveWorkspaceRoot(import.meta.dir)
	const Dir = ensureDir(import.meta.dir)
	const db = initProjectsDb(path.join(Dir, "aether.db"))
	ensureTasksTable(db)
	...
	const load = async (name: string, fn: () => Promise<any>) => {
		try { return await fn() } catch (e) { console.warn(`[aether-system] ${name} init failed:`, (e as Error).message); return null }
	}
	...
	console.log("[aether-system] Plugin initialized. Tools:", Object.keys(projectTools(mgr, db)).length, "project +", Object.keys(taskTools(db, mgr)).length, "task +", Object.keys(agentTools(client, db, mgr)).length, "agent")
	...
	return {
		...
		"experimental.session.compacting": async (_input: any, output: { context: string[] }) => {
			try {
				...
				output.context.push([
					`<aether_system type="tasks" source="aether-system">`,
