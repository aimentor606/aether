...
export function resolveWorkspaceRoot(anchorDir?: string): string {
	const explicitWorkspace = process.env.AETHER_WORKSPACE?.trim()
...
export function resolveDir(anchorDir?: string): string {
	const explicitDir = process.env.AETHER_DIR?.trim()
	if (explicitDir) return normalizeAbsolutePath(explicitDir)
	return join(resolveWorkspaceRoot(anchorDir), ".aether")
}

export function ensureDir(anchorDir?: string): string {
	const Dir = resolveDir(anchorDir)
...
/**
 * Ensure global USER.md + MEMORY.md exist at the workspace root .aether/ directory.
 * Deeper notes can live in .aether/memory/*.md and be referenced from MEMORY.md.
 */
export function ensureGlobalMemoryFiles(anchorDir?: string): { userPath: string; memoryPath: string; memoryDir: string } {
	const Dir = ensureDir(anchorDir)
	const memoryDir = join(Dir, "memory")
	mkdirSync(memoryDir, { recursive: true })
	return {
		userPath: ensureFile(join(Dir, "USER.md"), [
			"# User Profile",
			"",
			"## Preferences",
			"",
		...
		memoryPath: ensureFile(join(Dir, "MEMORY.md"), [
			"# Global Memory",
			"",
...
export function renderProjectContext(projectPath: string): string {
	const ctxPath = join(projectPath, ".aether", "CONTEXT.md")
