...
const scriptPath = "/Users/markokraemer/Projects/heyagi/computer/core/master/opencode/skills/KORTIX-system/connectors/integration.ts"
const pluginPath = "/Users/markokraemer/Projects/heyagi/computer/core/master/opencode/plugin/connectors/connectors.ts"
...
describe("connectors plugin + pipedream script e2e", () => {
  ...
  beforeEach(() => {
    ...
    dir = mkdtempSync(join(tmpdir(), "aether-connectors-"))
  })
...
describe("integration.ts end-to-end through master", () => {
  ...
  beforeAll(async () => {
    ...
    app.use("*", async (c, next) => {
      ...
      if (auth && auth !== "Bearer test-aether-token") {
    ...
    master = await startKortixMaster(masterPort, fx, {
      ...
      AETHER_TOKEN: "test-aether-token",
