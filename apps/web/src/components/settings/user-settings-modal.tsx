...
function GeneralTab({ onClose }: { onClose: () => void }) {
  ...
  const { data: deletionStatus, isLoading: isCheckingStatus } =
    useAccountDeletionStatus();
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Tooltip>
            ...
            <TooltipContent>{t('emailCannotChange')}</TooltipContent>
          </Tooltip>
        </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <Label className="text-sm">
                    {t('deleteAccount.chooseDeletionType')}
                  </Label>
                  <RadioGroup
                    value={deletionType}
                    onValueChange={(value) =>
                  ...
                  >
                    ...
                    <div className="flex items-start gap-2 sm:gap-3 rounded-md border border-red-500/30 p-3 sm:p-4">
                      ...
                      <div className="space-y-1 flex-1 min-w-0">
                        <Label
                          htmlFor="immediate"
                          className="font-medium cursor-pointer text-sm sm:text-base text-red-600 block"
                        >
            ...
            <AlertDialogContent className="max-w-md p-4 sm:p-6">
              <AlertDialogHeader>
                <AlertDialogTitle className="text-base sm:text-lg">
                  {t('deleteAccount.cancelDeletionTitle')}
                </AlertDialogTitle>
              </AlertDialogHeader>
              <div className="space-y-4">
...
function KeyboardShortcutsTab() {
  const { preferences, setKeyboardPreferences, getModifierLabel } =
  ...
  const events: { id: SoundEvent; label: string; description: string }[] = [
    {
      ...
      description: 'When AI finishes a task',
    },
    {
      id: 'error',
      label: 'Error',
        ...
        <div className="space-y-1.5">
          {instances.map((inst: any) => {
            const serverEntry = servers.find(
              (s) => s.sandboxId === inst.external_id,
            );
            const isActive = serverEntry
              ? activeServerId === serverEntry.id
              : false;
            const hasSub = Boolean(
              inst.stripe_subscription_id || inst.stripe_subscription_item_id,
            );
              ...
              >
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {isCancelling && (
                      <span className="text-[10px] px-1.5 py-0.5 rounded bg-red-500/10 text-red-600 dark:text-red-400 font-medium shrink-0">
                        Cancelling
                      </span>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-1.5 shrink-0 ml-3">
                  ...
                  {hasSub && !isCancelling && (
                    <Button
                      ...
                      disabled={loading === inst.sandbox_id}
                    >
...
function BillingTab({
  ...
  const hasScheduledChange =
    ...
    accountState?.subscription.scheduled_change;
  ...
  const handleManageSubscription = () => {
    console.log(
      '[BillingTab] Creating portal session with return_url:',
      returnUrl,
    );
  ...
  const isFreeTier =
    subscription?.tier_key === 'free' || subscription?.tier_key === 'none';
  const isCancelled =
    subscription?.is_cancelled || subscription?.cancel_at_period_end;
    ...
    <div className="p-4 sm:p-6 space-y-6 min-w-0 max-w-full overflow-x-hidden">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          Credits, instances, and subscription.
        </p>
      </div>
